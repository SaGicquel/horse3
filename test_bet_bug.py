...
print("\n✅ FORMULE CORRECTE:")
bankroll_correct = (
    bankroll_initial - stake + stake * odds if result == 1 else bankroll_initial - stake
)
profit_correct = stake * (odds - 1) if result == 1 else -stake
print(
    f"   Bankroll après pari: {bankroll_initial} - {stake} + {stake} × {odds} = {bankroll_correct}€"
)
...
print("   → Le bug ajoute la MISE en plus !")

print("\n🎯 CONCLUSION:")
print("   Le bug fait que chaque pari gagnant ajoute:")
print(f"   - Le profit NET: +{profit_correct}€")
print(f"   - PLUS la mise: +{stake}€")
print(f"   - Total ajouté: +{profit_bug}€ au lieu de +{profit_correct}€")
print("   → C'est pourquoi la bankroll explose !")
