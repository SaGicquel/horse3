...
class OptimizedMLFeatureEngineer:
    ...
    def add_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["rang_experience_cheval"] = df.groupby("id_course")["nb_courses_carriere"].rank(
            ascending=False
        )
        df["rang_taux_victoires_cheval"] = df.groupby("id_course")["taux_victoires_cheval"].rank(
            ascending=False
        )

        # Features d'interaction
        df["cote_x_experience"] = df["cote_log"] * np.log1p(df["nb_courses_carriere"])
        df["jockey_entraineur_synergie"] = (
            df["taux_victoires_jockey"] * df["taux_victoires_entraineur"]
        )
