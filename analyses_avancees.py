...
""")
    ...
    print(f"{'Rang':<5} | {'Nom':<30} | {'Victoires':<10} | {'Courses':<8} | {'Taux':<8} | {'Race':<20}")
    print(f"{'-'*5} | {'-'*30} | {'-'*10} | {'-'*8} | {'-'*8} | {'-'*20}")
    
    for i, (nom, vict, courses, taux, race) in enumerate(cur.fetchall(), 1):
        print(f"{i:<5} | {nom:<30} | {vict:<10} | {courses:<8} | {taux:<8.2f}% | {race or 'N/A':<20}")
...
""")
    ...
    print(f"{'Rang':<5} | {'Nom':<30} | {'Courses':<8} | {'Victoires':<10} | {'Taux':<8} | {'Musique':<15}")
    print(f"{'-'*5} | {'-'*30} | {'-'*8} | {'-'*10} | {'-'*8} | {'-'*15}")
    
    for i, (nom, courses, vict, taux, musique) in enumerate(cur.fetchall(), 1):
        musique_short = (musique[:15] + '...') if musique and len(musique) > 15 else (musique or 'N/A')
        print(f"{i:<5} | {nom:<30} | {courses:<8} | {vict:<10} | {taux:<8.2f}% | {musique_short:<15}")
...
""")
    ...
    print(f"{'Rang':<5} | {'Nom':<30} | {'Vict. Récentes':<15} | {'Sur':<5} | {'Total Courses':<15}")
...
""")
    ...
    cur.execute("SELECT COUNT(*) FROM chevaux WHERE race='TROTTEUR FRANCAIS' AND nombre_courses_2025 > 0")
    actifs_2025 = cur.fetchone()[0]
    
    cur.execute("SELECT SUM(nombre_courses_total), SUM(nombre_victoires_total) FROM chevaux WHERE race='TROTTEUR FRANCAIS'")
    total_courses, total_vict = cur.fetchone()
    
    cur.execute("SELECT AVG(CAST(nombre_victoires_total AS REAL) * 100 / nombre_courses_total) FROM chevaux WHERE race='TROTTEUR FRANCAIS' AND nombre_courses_total > 0")
