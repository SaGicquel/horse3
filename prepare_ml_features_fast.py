...
def calculate_cheval_stats(df):
    ...
    distance_stats = (
        df.groupby(["nom_norm", pd.cut(df["distance"], 5)])["victoire"].mean().reset_index()
    )
...
def calculate_jockey_entraineur_stats(df):
    ...
    jockey_stats["taux_victoires_jockey"] = (
        jockey_stats["jockey_victoires"] / jockey_stats["jockey_courses"]
    )
    jockey_stats["taux_places_jockey"] = (
        jockey_stats["jockey_places"] / jockey_stats["jockey_courses"]
    )

    df = df.merge(
        jockey_stats[["driver_jockey", "taux_victoires_jockey", "taux_places_jockey"]],
        on="driver_jockey",
        how="left",
    )
    ...
    # Stats entraineur
    entraineur_stats = (
        df.groupby("entraineur").agg({"victoire": ["count", "sum"], "place": "sum"}).reset_index()
    )
    entraineur_stats.columns = [
        "entraineur",
        "entraineur_courses",
        "entraineur_victoires",
        "entraineur_places",
    ]
    entraineur_stats["taux_victoires_entraineur"] = (
        entraineur_stats["entraineur_victoires"] / entraineur_stats["entraineur_courses"]
    )
    entraineur_stats["taux_places_entraineur"] = (
        entraineur_stats["entraineur_places"] / entraineur_stats["entraineur_courses"]
    )

    df = df.merge(
        entraineur_stats[["entraineur", "taux_victoires_entraineur", "taux_places_entraineur"]],
        on="entraineur",
        how="left",
    )
...
def prepare_final_dataset(df):
    ...
    encoding_cols = [
        ...
        if "discipline_" in col or "sexe_" in col or "etat_" in col or "hippodrome_" in col
    ]

    # Interactions
    interaction_cols = [
        col for col in df.columns if "interaction_" in col or col == "popularite_hippodrome"
    ]
    ...
    logger.info(
        f"   ✅ Dataset final : {len(df_final):,} lignes, {len(df_final.columns):,} colonnes"
    )
    return df_final
