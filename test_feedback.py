...
def test_submit_invalid_feedback() -> bool:
    ...
    try:
        ...
        if response.status_code == 422 or response.status_code == 400:
            ...
            print_colored(
                f"❌ Devrait rejeter feedback invalide, reçu: {response.status_code}", Colors.FAIL
            )
            return False
...
def test_model_performance() -> bool:
    ...
    try:
        # Tester avec 7 jours
        response = requests.get(f"{API_URL}/feedback/model-performance?days=7", timeout=TIMEOUT)
        ...
        if response.status_code == 200:
            ...
            print(
                f"   Accuracy top 1: {data['accuracy_top1']:.1%} ({data['nb_correct_top1']} corrects)"
            )
            print(
                f"   Accuracy top 3: {data['accuracy_top3']:.1%} ({data['nb_correct_top3']} corrects)"
            )
...
def test_model_performance_invalid_days() -> bool:
    ...
    try:
        ...
        response = requests.get(f"{API_URL}/feedback/model-performance?days=365", timeout=TIMEOUT)

        if response.status_code == 400:
            print_colored(
                "✅ Validation days fonctionne: paramètre invalide rejeté", Colors.OKGREEN
            )
...
def test_multiple_feedbacks() -> bool:
    ...
    try:
        ...
        if success_count == len(feedbacks):
            print_colored(
                f"✅ Tous les feedbacks enregistrés ({success_count}/{len(feedbacks)})",
                Colors.OKGREEN,
            )
            return True
        else:
