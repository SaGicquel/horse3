...
class DriftDetector:
    ...
    ML_FEATURES = [
        ...
        "specialite_MONTE",
        "specialite_PLAT",
        "specialite_OBSTACLES",
        "sexe_F",
        "sexe_H",
        "sexe_M",
        "discipline_ATTELE",
        "discipline_GALOP_PLAT",
        "discipline_GALOP_OBSTACLES",
        "discipline_TROT_ATTELE",
        "discipline_TROT_MONTE",
        "hippodrome_AUTEUIL",
        "hippodrome_CHANTILLY",
        "hippodrome_DEAUVILLE",
        "hippodrome_LONGCHAMP",
        "hippodrome_MAISONS-LAFFITTE",
        "hippodrome_SAINT-CLOUD",
        "hippodrome_VINCENNES",
        "type_piste_CORDE_DROITE",
        "type_piste_CORDE_GAUCHE",
        "type_piste_DROITE",
        "type_piste_LIGNE_DROITE",
        "categorie_course_APPRENTIS",
        "categorie_course_CONDITIONS",
        "categorie_course_GROUPE_1",
        "categorie_course_GROUPE_2",
        "categorie_course_GROUPE_3",
        "categorie_course_HANDICAP",
        "categorie_course_LISTED",
        "categorie_course_RECLAMERS",
    ...
    def compute_ks_test(
        self, baseline_values: np.ndarray, production_values: np.ndarray
    ) -> Tuple[float, float]:
        """
    ...
    def compute_js_divergence(
        self, baseline_values: np.ndarray, production_values: np.ndarray, bins: int = 50
    ) -> float:
        """
    ...
    def detect_feature_drift(
        self, feature_name: str, baseline_values: np.ndarray, production_values: np.ndarray
    ) -> Dict:
        """
        ...
        return {
            ...
            "severity": severity,
            "baseline_mean": round(float(baseline_clean.mean()), 4)
            if len(baseline_clean) > 0
            else 0.0,
            "production_mean": round(float(production_clean.mean()), 4)
            if len(production_clean) > 0
            else 0.0,
            "baseline_std": round(float(baseline_clean.std()), 4)
            if len(baseline_clean) > 0
            else 0.0,
            "production_std": round(float(production_clean.std()), 4)
            if len(production_clean) > 0
            else 0.0,
    ...
    def detect_all_features(
        self, df_baseline: pd.DataFrame, df_production: pd.DataFrame
    ) -> List[Dict]:
        ...
        for i, feature in enumerate(self.ML_FEATURES, 1):
            ...
            drift_info = self.detect_feature_drift(feature, baseline_values, production_values)
    ...
    def generate_report(
        self, drift_results: List[Dict], output_path: str = "drift_report.json"
    ) -> Dict:
        """
        ...
        report = {
            ...
            "drift_percentage": round(100 * features_with_drift / total_features, 2)
            if total_features > 0
            else 0.0,
...
Exemples:
    ...
    parser.add_argument(
        "--prometheus-output", type=str, help="Chemin pour export Prometheus (optionnel)"
    )
