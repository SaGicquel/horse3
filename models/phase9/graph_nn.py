...
class EntityGraphNN(nn.Module):
    ...
    """
    def __init__(self, metadata, hidden_dim: int = 64, out_dim: int = 1, n_layers: int = 3, dropout: float = 0.1):
        """
        Args:
...
class GNNPredictor(nn.Module):
    ...
    def gnn_forward_full(self, x_dict, edge_index_dict):
        ...
        for node_type, x in x_dict.items():
            ...
            x_dict_out[node_type] = F.relu(x_dict_out[node_type])
            x_dict_out[node_type] = F.dropout(x_dict_out[node_type], p=self.gnn.dropout, training=self.gnn.training)
