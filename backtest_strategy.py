...
def run_backtest(df, strategy_name, strategy_func, **kwargs):
    ...
    roi = ((final_balance - 1000) / (bets * 1.0)) * 100 # ROI approximatif (basé sur mise unitaire moyenne 1€)
...
def strategy_value_bet(group, min_edge=0.05, kelly_fraction=0.1):
    ...
    mise = max(0, kelly * kelly_fraction * 1000) # Mise basée sur bankroll fixe 1000 pour simplifier
...
def main():
    ...
    if not pd.io.common.file_exists(PREDICTIONS_PATH):
        logger.error(f"❌ Fichier {PREDICTIONS_PATH} introuvable. Lancez generate_predictions_phase10.py d'abord.")
        return
    ...
    res_flat_secure = run_backtest(df, "Flat Betting (Top 1, Conf>50%)", strategy_flat_top1, threshold=0.5)
    ...
    if pd.io.common.file_exists(CALIBRATED_PATH):
        ...
        res_value_calib = run_backtest(df_calib, "Value Betting (Kelly 10% - Calibré)", strategy_value_bet, min_edge=0.02)
