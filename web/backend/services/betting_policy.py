...
def select_portfolio_from_picks(
    *,
    ...
    pconf = profiles.get(profile, profiles.get("STANDARD", {})) if isinstance(profiles, dict) else {}

    zones = policy.get("bankroll_zones", {}) if isinstance(policy.get("bankroll_zones", {}), dict) else {}
    ...
    value_map = policy.get("value_min_pct_by_zone", {}) if isinstance(policy.get("value_min_pct_by_zone", {}), dict) else {}
    zone_map = value_map.get(zone, {}) if isinstance(value_map.get(zone, {}), dict) else {}
    profile_mult_map = policy.get("profile_value_multiplier", {}) if isinstance(policy.get("profile_value_multiplier", {}), dict) else {}
    profile_mult = float(profile_mult_map.get(profile, profile_mult_map.get("STANDARD", 1.0)) or 1.0)

    stake_scale_bucket = policy.get("stake_scale_by_bucket", {}) if isinstance(policy.get("stake_scale_by_bucket", {}), dict) else {}
    stake_scale_zone = policy.get("stake_scale_by_zone", {}) if isinstance(policy.get("stake_scale_by_zone", {}), dict) else {}
    ...
    for pick in picks:
        ...
        if bet_risk not in allowed_risks:
            excluded.append({**pick, "excludeReason": f"risque {bet_risk} exclu (profil {profile})"})
            continue
        ...
        if bmap and kind in bmap:
            ...
            # fallback: valeur conservatrice
            cutoff = (8.0 if kind == "win" else 6.0) * profile_mult

        if is_place:
            if value_pct < cutoff:
                excluded.append({**pick, "excludeReason": f"value_place {value_pct:.1f}% < seuil {cutoff:.1f}%"})
                continue
        else:
            if cote and cote > max_odds_win:
                excluded.append({**pick, "excludeReason": f"cote {cote:.1f} > max {max_odds_win:.0f} (WIN)"})
                continue
            if value_pct < cutoff:
                excluded.append({**pick, "excludeReason": f"value {value_pct:.1f}% < seuil {cutoff:.1f}%"})
                continue
    ...
    for cand in candidates:
        ...
        stake_rate = min(max(0.0, kelly_raw * float(kelly_fraction or 0.0)), float(cap_per_bet or 0.0))
        ...
        if daily_budget > 0 and total_stake + stake > daily_budget:
            ...
            remaining = math.floor(remaining / rounding) * rounding if rounding > 0 else round(remaining, 2)
            if remaining <= 0:
    ...
    return {
        ...
        "policy": {
            ...
            "allowed_bet_types": sorted(list(allowed_bet_types)) if allowed_bet_types is not None else None,
            "profile_value_multiplier": profile_mult,
            "value_min_pct_by_zone": value_map.get(zone) if isinstance(value_map.get(zone), dict) else None,
