...
def analyser_champs_manquants():
    ...
    for col in colonnes_vides:
        ...
        elif any(k in name for k in ['driver', 'jockey', 'entraineur', 'proprietaire', 'eleveur', 
                                       'age', 'sexe', 'robe', 'race', 'poids', 'equipement', 'deferrage']):
            categories['participant'].append(col)
        elif any(k in name for k in ['rapport', 'enjeux', 'allocation']):
            categories['rapports'].append(col)
        elif any(k in name for k in ['place', 'temps', 'vitesse', 'ecart', 'gains', 'statut_arrivee']):
        ...
        elif any(k in name for k in ['type_', 'classe', 'profil', 'parcours', 'commentaire', 'prix_course']):
    ...
    categories_ordre = ['course', 'hippodrome', 'participant', 'resultats', 'rapports', 'meteo', 'identifiants', 'autres']
    ...
    for cat_name in categories_ordre:
        ...
        for col in cols:
            emoji = '✅' if col['champ_api'] != '❓ NON IDENTIFIÉ' and '✅' in col['champ_api'] else '🔴' if col['pct_vide'] > 90 else '🟠'
