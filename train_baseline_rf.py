...
class BaselineModel:
    ...
    def load_data(self, input_dir: Path):
        ...
        df_val = (
            pd.read_parquet(val_path) if val_path.suffix == ".parquet" else pd.read_csv(val_path)
        )
        ...
        feature_cols = [
            ...
            if c not in exclude_cols and df_train[c].dtype in ["int64", "float64", "bool"]
        ]
        ...
        logger.info(
            f"🎯 Target distribution train : {y_train.sum():,} victoires ({100*y_train.mean():.2f}%)"
        )
        logger.info(
            f"🎯 Target distribution val : {y_val.sum():,} victoires ({100*y_val.mean():.2f}%)"
        )
        logger.info("")
...
def main():
    ...
    parser.add_argument(
        "--input-dir", default="data/normalized", help="Répertoire avec données normalisées"
    )
    parser.add_argument("--output-dir", default="data/models", help="Répertoire de sortie")
    parser.add_argument("--n-estimators", type=int, default=100, help="Nombre d'arbres")
    parser.add_argument("--max-depth", type=int, default=10, help="Profondeur max")

    args = parser.parse_args()

    try:
        # Initialiser
        model = BaselineModel(n_estimators=args.n_estimators, max_depth=args.max_depth)
