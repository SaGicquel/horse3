...
def view_top_chevaux():
    ...
    if rows:
        ...
        for row in rows:
            ...
            print(f"   {row[0]:25s} {row[1]:1s} {row[2]:4d} {row[3]:7d} {row[4]:5d} {row[5]:6.1f} {forme:6s}")
    else:
...
def view_top_jockeys():
    ...
    if rows:
        ...
        for row in rows:
            print(f"   {row[0]:30s} {row[1]:7d} {row[2]:5d} {row[3]:6.1f} {row[4]:6d} {row[5]:6.1f}")
    else:
        print("   (aucune statistique calculée)")
...
def view_recent_performances():
    ...
    if rows:
        ...
        for row in rows:
            ...
            print(f"   {str(row[0]):10s} {row[1]:15s} {row[2]:8s} {row[3]:20s} {pos:3s} {cote:6s} {jockey:20s}")
    else:
