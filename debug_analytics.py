...
try:
    ...
    if is_postgres:
        cur.execute(
            "SELECT COUNT(*) FROM (SELECT driver_jockey FROM cheval_courses_seen GROUP BY driver_jockey HAVING COUNT(*) >= 10) as t"
        )
    else:
        cur.execute(
            "SELECT COUNT(*) FROM (SELECT jockey_habituel FROM chevaux GROUP BY jockey_habituel HAVING COUNT(*) >= 3) as t"
        )
...
try:
    ...
    if is_postgres:
        cur.execute(
            "SELECT COUNT(*) FROM (SELECT entraineur FROM cheval_courses_seen GROUP BY entraineur HAVING COUNT(*) >= 10) as t"
        )
    else:
        cur.execute(
            "SELECT COUNT(*) FROM (SELECT entraineur_courant FROM chevaux GROUP BY entraineur_courant HAVING COUNT(*) >= 3) as t"
        )
