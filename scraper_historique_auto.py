...
class DataQualityChecker:
            ...
            self.issues.append(
                {
                    ...
                    "examples": [
                        {"nom": d[0], "annee": d[1], "count": d[2]} for d in duplicates[:3]
                    ],
                }
            )
    ...
    def _check_duplicate_courses(self) -> Dict:
        ...
        return {"status": "ok" if not duplicates else "error", "duplicates_found": len(duplicates)}
...
class HistoricalScraper:
    ...
    def initialize(self, years: Optional[int] = None):
        ...
        logger.info("🚀 Scraping historique initialisé")
    ...
    def scrape_next_period(self) -> Optional[Dict]:
        ...
        logger.info(f"\n{'='*70}")
        logger.info(
            f"📅 PÉRIODE: {period_start} → {period_end} ({(period_end - period_start).days + 1} jours)"
        )
        logger.info(f"{'='*70}")
        ...
        try:
            ...
            while current_date >= period_start:
                ...
                if self.consecutive_errors >= MAX_ERRORS_BEFORE_PAUSE:
                    logger.warning(
                        f"⚠️ {MAX_ERRORS_BEFORE_PAUSE} erreurs consécutives - Pause de {PAUSE_DURATION_MINUTES} minutes"
                    )
            ...
            if quality_report["issues"]:
                for issue in quality_report["issues"]:
                    ...
                    logger.warning(
                        f"   {severity_icon} {issue['type']}: {issue.get('message', issue.get('count', ''))}"
                    )
            else:
            ...
            self.progress["total_courses"] = (
                self.progress.get("total_courses", 0) + period_results["total_courses"]
            )
            self.progress["total_chevaux"] = (
                self.progress.get("total_chevaux", 0) + period_results["total_chevaux"]
            )
            self.progress["total_errors"] = self.progress.get("total_errors", 0) + len(
                period_results["errors"]
            )
            ...
            logger.info(
                f"   • Qualité: {'✅ OK' if quality_report['passed'] else '⚠️ Problèmes détectés'}"
            )
    ...
    def run_continuous(self, max_periods: Optional[int] = None):
        ...
        while True:
            ...
            logger.info(
                f"⏳ Pause de {DELAY_BETWEEN_PERIODS} secondes avant la prochaine période..."
            )
