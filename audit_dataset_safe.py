...
def audit_dataset_safe():
    ...
    try:
        ...
        logging.info(f"✅ Dataset SAFE chargé: {df_safe.shape[0]:,} lignes × {df_safe.shape[1]} colonnes")
        logging.info(f"✅ Ancien dataset chargé: {df_old.shape[0]:,} lignes × {df_old.shape[1]} colonnes")
        ...
        mots_suspects = [
            'victoire', 'place', 'position', 'arrivee', 'resultat',
            'classement', 'rang', 'gagnant', 'performance',
            'temps_final', 'ecart_vainqueur'
        ]
        ...
        anciennes_fuites = [
            'nb_victoires_carriere', 'taux_victoires_cheval',
            'nb_places_carriere', 'taux_places_cheval',
            'nb_victoires_jockey', 'taux_victoires_jockey',
            'nb_places_jockey', 'taux_places_jockey',
            'moyenne_position_cheval', 'moyenne_position_jockey'
        ]
        ...
        if len(colonnes_communes) > 0:
            ...
            common_cols = ['id_course', 'num_cheval'] if all(c in colonnes_communes for c in ['id_course', 'num_cheval']) else list(colonnes_communes)[:5]
            ...
            for col in common_cols[:3]:  # Limiter pour lisibilité
                if col in df_safe.columns and col in df_old.columns:
                    ...
                    if df_safe[col].dtype in ['int64', 'float64'] and df_old[col].dtype in ['int64', 'float64']:
