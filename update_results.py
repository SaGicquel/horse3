...
def update_pnl():
    ...
    for idx, row in pending_bets.iterrows():
        ...
        if result:
            ...
            # Mise à jour
            if position == 1:
                gain = mise * (cote_sp if cote_sp else 1.0) # Fallback cote 1 si manquante (ne devrait pas arriver)
                df.at[idx, 'statut'] = 'Gagné'
                df.at[idx, 'gain'] = gain - mise # Net profit
                logger.info(f"💰 Gagné! Course {course_id} Cheval {cheval_id} -> +{gain - mise:.2f}€")
            else:
