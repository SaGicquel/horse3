...
def blend_logit_odds(
    ...
    n = len(p_model)
    if n != len(p_market):
        raise ValueError(f"p_model et p_market doivent avoir la même taille: {n} vs {len(p_market)}")
...
class RacePronosticGenerator:
    ...
    def __init__(self, db_connection, config: dict = None):
        ...
        logger.info(f"RacePronosticGenerator init: config_source={source}, "
    ...
    def generate_pronostic(
        ...
        for i, info in enumerate(runner_info):
            ...
            # Value
            value_win = self.kelly.calculate_value(
                p_win, 
            ...
            # Kelly
            kelly_win = self.kelly.calculate(
                p_win,
                odds if odds else 10.0,
            ...
            bucket = classify_runner(
                p_win, 
                value_win if value_win else -1,
                variance,
                odds if odds else 100
            )
...
class TestBlendLogitOdds(unittest.TestCase):
    ...
    def test_normalization_sum_one(self):
        ...
        self.assertAlmostEqual(np.sum(result), 1.0, places=10,
                               msg="Somme des probabilités doit être 1")
    ...
    def test_alpha_zero_equals_market(self):
        ...
        np.testing.assert_array_almost_equal(result, expected, decimal=5,
                                             err_msg="α=0 devrait donner les probas marché")
    ...
    def test_alpha_one_equals_model(self):
        ...
        np.testing.assert_array_almost_equal(result, expected, decimal=5,
                                             err_msg="α=1 devrait donner les probas modèle")
    ...
    def test_shrinkage_extreme_divergence(self):
        ...
        self.assertTrue(result[0] < 0.8, 
                       f"Shrinkage devrait modérer la confiance extrême: {result[0]}")
    ...
    def test_discipline_alpha_selection(self):
        ...
        for disc in expected_disciplines:
            self.assertIn(disc, alpha_by_disc, 
                         f"Alpha pour discipline '{disc}' devrait être défini")
