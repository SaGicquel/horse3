...
class HarvilleEstimator:
    ...
    def _simulate_position_prob(self, horse_idx: int, position: int, n_sim: int = 10000) -> float:
    ...
    def p_trifecta(self, i: int, j: int, k: int) -> float:
        ...
        if rem1 < 1e-10 or rem2 < 1e-10:
            return 0.0
        return self.win_probs[i] * self.win_probs[j] / rem1 * self.win_probs[k] / rem2
...
class LoBaconShoneEstimator:
    ...
    def __init__(self, win_probs: np.ndarray, iterations: int = 100, damping: float = 0.7):
        """
        Args:
            win_probs: Probabilités de victoire
    ...
    def p_trifecta(self, i: int, j: int, k: int) -> float:
        ...
        top_probs = sorted([self.win_probs[i], self.win_probs[j], self.win_probs[k]], reverse=True)
...
class PlackettLuceTemperatureSimulator:
    ...
    def __init__(self, win_probs: np.ndarray, temperature: float = 1.0, seed: Optional[int] = None):
        """
        Args:
    ...
    def estimate_combo_probs(self, n_sim: int, structure: str = "trio") -> Dict[Tuple, float]:
        ...
        structure_map = {"exacta": 2, "couple": 2, "trio": 3, "tierce": 3, "quarte": 4, "quinte": 5}
...
class PlaceEstimatorSelector:
    ...
    def get_simulator(
        self, win_probs: np.ndarray, discipline: str = "default", seed: Optional[int] = None
    ...
    def validate_estimator(
        self, estimator_name: str, historical_data: List[Dict], discipline: str
    ) -> Dict[str, float]:
        """
...
class CalibrationMetrics:
    ...
    def check_monotonicity(
        probs: np.ndarray, outcomes: np.ndarray, n_deciles: int = 10
    ) -> Dict[str, Any]:
        """
        ...
        is_monotonic = all(
            decile_rates[i] <= decile_rates[i + 1] for i in range(len(decile_rates) - 1)
        )
...
class EVStabilityValidator:
    ...
    def check_stability(
        self, n_values: List[int] = None, structure: str = "trio"
    ) -> Dict[str, Any]:
        """
        ...
        for i, n in enumerate(n_values):
            if i >= 2:  # Au moins 3 points
                ...
                cv = (
                    np.std(recent_means) / abs(np.mean(recent_means))
                    if np.mean(recent_means) != 0
                    else float("inf")
                )
                if cv < 0.10:
                    stable_n = n
...
class PlaceProbabilityEstimator:
    ...
    def __init__(
        ...
        config: PlaceEstimatorConfig = None,
    ...
    def estimate_place_probs(self, top_n: int = 3, method: str = "auto") -> Dict[str, Any]:
        """
        ...
        result = {
            ...
            "by_horse": {
                self.horse_names[i]: {
                    ...
                    "place_vs_win_ratio": round(float(p_place[i] / self.p_win[i]), 2)
                    if self.p_win[i] > 0.001
                    else 0,
                }
    ...
    def estimate_combo_probs(self, structure: str = "trio", n_sim: int = None) -> Dict[str, Any]:
        """
    ...
    def calculate_tickets_ev(
        self,
        combo_probs: Dict[Tuple, float] = None,
        structure: str = "trio",
        ...
        # Calculer EV
        tickets_ev = self.ev_calculator.calculate_tickets_ev(combo_probs, fixed_odds)
    ...
    def generate_report(
        self, structure: str = "trio", include_validation: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        if include_validation:
            ...
            stability = validator.check_stability(
                n_values=[5000, 10000, 20000], structure=structure
            )
            report["validation"] = {
    ...
    def compare_estimators(self) -> Dict[str, Any]:
        ...
        for name, est in estimators.items():
            ...
            results[name] = {
                "p_place": {
                    self.horse_names[i]: round(float(p_place[i]), 4) for i in range(self.n_horses)
                },
...
class TemperatureLearner:
    ...
    def _log_likelihood(
        self, temperature: float, win_probs: np.ndarray, actual_order: List[int]
    ) -> float:
    ...
    def learn_temperature(
        self, discipline: str, temp_range: Tuple[float, float] = (0.5, 2.0)
    ) -> Dict[str, Any]:
        """
        ...
        races = [r for r in self.races if r.get("discipline", "").lower() == discipline.lower()]
        ...
        def neg_total_ll(temp):
            total = 0
            for race in races:
                ll = self._log_likelihood(temp, np.array(race["win_probs"]), race["actual_order"])
                total += ll
            return -total
...
def estimate_exotic_probs(
    p_win_blend: List[float],
    discipline: str = "plat",
    structure: str = "trio",
    ...
    estimator = PlaceProbabilityEstimator(
        np.array(p_win_blend), discipline=discipline, config=config, horse_names=horse_names
    )

    return estimator.generate_report(structure=structure, include_validation=False)


def compare_estimators_on_race(p_win: List[float], horse_names: List[str] = None) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    # Créer l'estimateur
    estimator = PlaceProbabilityEstimator(p_win, discipline="plat", horse_names=horse_names)
