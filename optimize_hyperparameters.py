...
def optimize_model(
    ...
    study.optimize(
        ...
        callbacks=[
            ...
            ) if (trial.number + 1) % 10 == 0 else None
        ]
    )
    ...
    logger.info(f"   ✂️  Trials pruned: {len([t for t in study.trials if t.state == optuna.trial.TrialState.PRUNED])}")
    logger.info("")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='Nombre de folds pour TimeSeriesSplit (default: 5)'
    )
