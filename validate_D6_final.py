...
def validate_phase_d6():
    ...
    logger.info(
        f"📊 Courses parfaitement normalisées: {perfect_normalization:,}/{total_races:,} ({pct_normalized:.1f}%)"
    )
    ...
    if pct_normalized >= 99.0:
        ...
        logger.warning("⚠️  Normalisation problématique (< 95%)")
    ...
    for col in ["p_model_win", "p_model_norm", "p_calibrated", "p_final"]:
        if col in df.columns:
            ...
            logger.info(f"📊 {col}:")
            logger.info(
                f"   Min: {stats['min']:.6f}, Médiane: {stats['50%']:.4f}, Max: {stats['max']:.4f}"
            )
    ...
    if all(col in df.columns for col in ["p_model_win", "p_final"]):
        ...
        if corr_orig_final >= 0.95:
            ...
            logger.warning("⚠️  Corrélation faible (< 0.90)")
    ...
    logger.info(
        f"📊 Période couverte: {date_min.strftime('%Y-%m-%d')} à {date_max.strftime('%Y-%m-%d')}"
    )

    # Distribution par split
    split_dist = df["split"].value_counts()
    logger.info("📊 Distribution splits:")
    ...
    logger.info(
        f"🔄 Normalisation: {pct_normalized:.1f}% courses {'✅' if pct_normalized >= 95 else '⚠️'}"
    )
    logger.info("📂 Fichier final: data/backtest_predictions_calibrated.csv")
    logger.info("📦 Artefacts: calibration/ (température, calibrateur, blender)")
