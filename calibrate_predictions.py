...
def main():
    ...
    logger.info(f"📉 Brier Score Après: {brier_after:.5f} ({'✅ Amélioration' if brier_after < brier_before else '❌ Dégradation'})")
    ...
    logger.info(f"{'Bin':<10} | {'Count':<6} | {'Pred Avg':<10} | {'True Avg':<10} | {'Calib Avg':<10}")
    logger.info("-" * 60)
    ...
    for i in range(len(bins)-1):
        ...
        if mask.sum() > 0:
            ...
            logger.info(f"{bins[i]:.1f}-{bins[i+1]:.1f} | {count:<6} | {pred_avg:.4f}     | {true_avg:.4f}     | {calib_avg:.4f}")
