...
# ============================================================================
# CHARGEUR DE VRAIES PRÉDICTIONS CALIBRÉES
# ============================================================================


def load_real_predictions(
    path: str | Path = "data/backtest_predictions_calibrated.csv",
    config: "BacktestConfig" | None = None,
) -> pd.DataFrame:
    """
    Charge les vraies prédictions calibrées depuis le fichier CSV.

    Args:
        path: Chemin vers le fichier de prédictions calibrées
        config: Configuration de backtest (pour filtrage temporel optionnel)

    Returns:
        DataFrame avec les prédictions nettoyées et triées
    ...
    path = Path(path)
    if not path.exists():
        raise FileNotFoundError(f"Fichier de prédictions introuvable: {path}")

    # 1) Chargement
    print(f"📂 Chargement des prédictions depuis: {path}")
    df = pd.read_csv(path)
    print(f"   Données brutes: {len(df):,} entrées")

    # 2) Conversions utiles
    if "date_course" in df.columns:
        df["date_course"] = pd.to_datetime(df["date_course"])
    else:
        raise ValueError("Colonne 'date_course' manquante dans le fichier calibré")

    # 3) Filtre temporel (optionnel mais conseillé)
    if config is not None:
        test_start = pd.to_datetime(config.test_start)
        test_end = pd.to_datetime(config.test_end)

        mask = (df["date_course"] >= test_start) & (df["date_course"] <= test_end)
        df = df.loc[mask].copy()
        print(
            f"   Après filtre temporel {config.test_start} → {config.test_end}: {len(df):,} entrées"
        )

    # 4) Nettoyage minimum : proba + cote dispo
    required_cols = ["p_final", "cote_sp", "is_win", "race_key", "id_cheval"]
    for col in required_cols:
        if col not in df.columns:
            raise ValueError(f"Colonne requise manquante: {col}")

    df = df.dropna(subset=["p_final", "cote_sp"])
    print(f"   Après nettoyage des NaN: {len(df):,} entrées")

    # 5) Tri chronologique
    df = df.sort_values(["date_course", "race_key"]).reset_index(drop=True)

    print(f"   Période finale: {df['date_course'].min()} → {df['date_course'].max()}")
    print(f"   Courses: {df['race_key'].nunique():,}")
...
class BacktestConfig:
    ...
    max_absolute_stake: float = 100.0  # CAP ABSOLU: Max 100€ par pari
    min_stake: float = 2.0
...
class BetSimulator:
    ...
    def place_bet(
        ...
        # 🚨 NOUVEAU: Cap absolu pour éviter l'explosion des mises
        stake = min(stake, self.config.max_absolute_stake)
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        if len(df_placed) >= 10:
            df_placed["value_decile"] = pd.qcut(
                df_placed["ev_expected"], q=10, labels=False, duplicates="drop"
            )
        ...
        daily_exposure_stats = {
            ...
            "days_at_cap": sum(
                1
                for exp in self.daily_exposure.values()
        ...
        return {
            ...
            "bankroll_growth_pct": round(
                (self.bankroll - self.initial_bankroll) / self.initial_bankroll * 100, 2
            ),
    ...
    def _analyze_drawdown_series(self) -> Dict[str, Any]:
        ...
        for i, d in enumerate(dd):
            if d > 0.01:
                ...
                if current_period > 0:
                    drawdown_periods.append({"length": current_period, "max_dd": current_max})
                current_period = 0
                current_max = 0
        ...
        return {
            ...
            "longest_drawdown_period": max(p["length"] for p in drawdown_periods)
            if drawdown_periods
            else 0,
...
class CalibrationAnalyzer:
    ...
    def compute_calibration(self, probs: np.ndarray, outcomes: np.ndarray) -> Dict[str, Any]:
...
class StrategyBacktester:
    ...
    def _adapt_real_data_format(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Adapte le format des vraies données calibrées au format attendu par le pipeline.

        Args:
            df: DataFrame des prédictions calibrées

        Returns:
            DataFrame adapté pour le backtesting
        """
        print("🔧 Adaptation du format des données...")
        ...
        column_mapping = {
            ...
            "cote_sp": "odds_preoff",
            "is_win": "label_win",
            "is_place": "label_place",
            "p_final": "p_blend",  # Utiliser p_final comme probabilité principale
        }

        # Renommer les colonnes
        for old_col, new_col in column_mapping.items():
            if old_col in df_adapted.columns:
                df_adapted = df_adapted.rename(columns={old_col: new_col})

        # Créer les colonnes manquantes si nécessaire
        if "discipline" not in df_adapted.columns:
            df_adapted["discipline"] = "plat"  # Valeur par défaut

        if "n_runners" not in df_adapted.columns:
            df_adapted["n_runners"] = df_adapted.groupby("race_id")["race_id"].transform("count")

        # Créer logits_model à partir de p_final pour compatibilité
        if "logits_model" not in df_adapted.columns and "p_blend" in df_adapted.columns:
            # Convertir les probabilités en logits (méthode simplifiée)
            probs = np.clip(df_adapted["p_blend"].values, 1e-10, 1 - 1e-10)
            df_adapted["logits_model"] = np.log(probs / (1 - probs))

        # Vérifier les colonnes requises
        required_columns = ["date", "race_id", "horse_id", "odds_preoff", "label_win", "p_blend"]
        missing_columns = [col for col in required_columns if col not in df_adapted.columns]

        if missing_columns:
            raise ValueError(f"Colonnes manquantes après adaptation: {missing_columns}")

        # Filtrer les disciplines si spécifié
        if hasattr(self.config, "disciplines") and self.config.disciplines:
            if "discipline" in df_adapted.columns:
                mask = df_adapted["discipline"].isin(self.config.disciplines)
                df_adapted = df_adapted[mask].copy()
                print(
                    f"   Après filtre disciplines {self.config.disciplines}: {len(df_adapted):,} entrées"
                )

        # Filtrer les cotes
        if hasattr(self.config, "min_odds") and hasattr(self.config, "max_odds"):
            mask = (df_adapted["odds_preoff"] >= self.config.min_odds) & (
                df_adapted["odds_preoff"] <= self.config.max_odds
            )
            df_adapted = df_adapted[mask].copy()
            print(
                f"   Après filtre cotes [{self.config.min_odds}-{self.config.max_odds}]: {len(df_adapted):,} entrées"
            )

        print(f"✅ Adaptation terminée: {len(df_adapted):,} entrées")
        return df_adapted

    def _normalize_probabilities(
        self, df: pd.DataFrame, col: str, race_col: str = "race_id"
    ) -> pd.DataFrame:
    ...
    def _compute_model_probs(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Utilise les probabilités déjà calibrées depuis le fichier.
        Pour compatibilité avec le pipeline existant, crée aussi p_model et p_market.
        """
        df = df.copy()

        # Vérifier si on a déjà p_blend (from p_final in adaptation)
        if "p_blend" in df.columns:
            print("✅ Utilisation des probabilités calibrées p_final -> p_blend")

            # Pour compatibilité, reconstruire p_model et p_market si pas présents
            if "p_market" not in df.columns:
                df["p_market"] = 1 / df["odds_preoff"]
                df = self._normalize_probabilities(df, "p_market")

            if "p_model" not in df.columns:
                # Approximation : p_model ≈ p_blend (déjà calibré et blendé)
                df["p_model"] = df["p_blend"].copy()

        else:
            # Fallback : calcul classique si pas de prédictions calibrées
            print("⚠️ Fallback : calcul des probabilités depuis logits_model")
            ...
            df["p_blend"] = blend_logits(
                df["p_model"].values, df["p_market"].values, self.config.blend_alpha
            )
            df = self._normalize_probabilities(df, "p_blend")

        # EV basées sur les vraies probabilités calibrées
    ...
    def _run_strategy(
        ...
        if len(value_bets) == 0:
            return {"strategy": strategy_name, "n_bets": 0, "error": "No value bets found"}
    ...
    def run_backtest(self, df: pd.DataFrame = None) -> Dict[str, Any]:
        ...
        print(
            f"   Cap journalier: {self.config.daily_budget_cap}€ ou {self.config.daily_budget_cap_pct*100}%"
        )
        ...
        # Charger les vraies prédictions calibrées
        if df is None:
            print("\n📂 Chargement des vraies prédictions calibrées...")
            df = load_real_predictions(config=self.config)

            # Adapter les colonnes pour compatibilité avec le reste du pipeline
            df = self._adapt_real_data_format(df)
        ...
        print(
            f"   Val:   {len(val_df)} ({val_df['date'].min() if len(val_df) > 0 else 'N/A'} → {val_df['date'].max() if len(val_df) > 0 else 'N/A'})"
        )
        print(
            f"   Test:  {len(test_df)} ({test_df['date'].min() if len(test_df) > 0 else 'N/A'} → {test_df['date'].max() if len(test_df) > 0 else 'N/A'})"
        )
        ...
        result = self._run_strategy(
            test_df, "Marché Pur", "p_market", use_kelly=True, value_cutoff=-0.10
        )
        ...
        for name, result in strategies_results.items():
            if "probs" in result and len(result["probs"]) > 0:
                cal = calibration_analyzer.compute_calibration(result["probs"], result["outcomes"])
        ...
        final_results = {
            ...
            "detailed_metrics": {
                name: result.get("metrics", {}) for name, result in strategies_results.items()
            },
    ...
    def _generate_all_plots(
        ...
        ax.axvline(
            x=self.config.value_cutoff * 100,
            color="red",
            linestyle="--",
            label=f"Cutoff ({self.config.value_cutoff*100}%)",
        )
...
class TestStrategyBacktester(unittest.TestCase):
    ...
    def test_negative_ev_stake_zero(self):
        ...
        self.assertEqual(
            result["stake"], 0.0, f"Stake devrait être 0 pour EV négatif, obtenu: {result['stake']}"
        )
        self.assertEqual(
    ...
    def test_suspicious_columns_detected(self):
        ...
        self.assertTrue(
            len(backtester._leak_warnings) > 0, "Des warnings de fuite auraient dû être générés"
        )
...
def main():
    ...
    if args.test:
        ...
        # Configuration pour backtest sur 5 ans complets
        config = BacktestConfig(
            train_start="2020-11-27",  # Début des données calibrées
            train_end="2023-06-30",
            val_start="2023-07-01",
            val_end="2023-08-31",
            test_start="2020-11-27",  # Utiliser TOUTES les données calibrées pour le test
            test_end="2025-12-08",  # Fin des données calibrées
            initial_bankroll=1000.0,
            kelly_fraction=0.25,
            max_stake_pct=0.02,  # 🚨 2% au lieu de 5% par défaut
            value_cutoff=0.05,
            blend_alpha=0.4,
            max_absolute_stake=100.0,  # 🚨 CAP ABSOLU AJOUTÉ
            slippage_ticks=args.slippage_ticks,
            daily_budget_cap=args.daily_cap,
            output_dir=args.output,
            strict_temporal_split=False,  # Désactiver pour utiliser toutes les données
            leak_detection_enabled=False,  # Désactiver la détection de fuite
        )
        ...
        print("-" * 100)
        print(
            f"{'Stratégie':<25} {'N Paris':>8} {'Skip':>6} {'ROI %':>10} {'Sharpe':>10} {'Max DD %':>10} {'Turnover':>10}"
        )
        print("-" * 100)

        for s in results["strategies_comparison"]:
            print(
                f"{s['strategy']:<25} {s['n_bets']:>8} {s.get('n_bets_skipped', 0):>6} {s['roi_pct']:>10.2f} {s['sharpe']:>10.3f} {s['max_dd_pct']:>10.2f} {s.get('turnover', 0):>10.2f}"
            )
