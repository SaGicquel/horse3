...
MAX_WORKERS = 8  # Nombre de threads parallèles (ajustez selon votre connexion)
RATE_LIMIT_DELAY = 0.1  # Délai réduit entre requêtes (multi-thread = moins de pression par thread)
...
def get_json(url: str, timeout=15):
    """
    GET JSON robuste (évite JSONDecodeError si HTML/empty).
    """
    ...
    if "text/html" in ctype or txt.startswith("<!DOCTYPE html") or txt.startswith("<html"):
        ...
        print(f"[WARN] Non-JSON response ({r.status_code}) from {url}: {head}")
        return None
    try:
        return r.json()
    except requests.JSONDecodeError:
        head = textwrap.shorten(txt.replace("\n", " "), width=180)
        print(f"[WARN] JSON parse failed for {url}: {head}")
        return None
...
def map_sexe(s: str|None) -> str|None:
    if not s: return None
    u = s.upper()
    if "HONGRE" in u: return "H"
    if "FEMEL"  in u: return "F"
    if "MALE"   in u or u == "M": return "M"
    return None
        ...
        for item in rapports_data:
            ...
            if rapports and len(rapports) > 0:
                ...
                if isinstance(premier_rapport, dict):
                    dividende = premier_rapport.get("dividende") or premier_rapport.get("dividendePourUnEuro")
                    if not dividende:
    ...
    # Format alternatif : dict direct (ancien format)
        ...
        if quarte:
            if isinstance(quarte, dict):
                ...
                result["montant_enjeux_quarte"] = quarte.get("montantEnjeux") or quarte.get("enjeux")
            else:
        ...
        if quinte:
            if isinstance(quinte, dict):
                ...
                result["montant_enjeux_quinte"] = quinte.get("montantEnjeux") or quinte.get("enjeux")
            else:
        ...
        # Montant total
        total_enjeux = rapports_data.get("montantEnjeuxTotal") or rapports_data.get("enjeuxTotal") or rapports_data.get("montantTotal")
        if total_enjeux:
...
def extract_info_hippodrome(hippo_data) -> dict:
    ...
    if isinstance(hippo_data, dict):
        result["pays_hippodrome"] = (
            hippo_data.get("pays")
            or hippo_data.get("country")
            or hippo_data.get("codePays")
        )
        result["region_hippodrome"] = (
            hippo_data.get("region")
            or hippo_data.get("departement")
            or hippo_data.get("zone")
        )
...
def extract_info_personnel(personnel_data) -> dict:
    ...
    if isinstance(personnel_data, dict):
        # ID PMU
        result["id_pmu"] = personnel_data.get("id") or personnel_data.get("idPmu") or personnel_data.get("numPmu")
...
def extract_allocations_detaillees(course_data: dict, place_finale: int) -> dict:
    ...
    if isinstance(repartition, dict):
        result["allocation_premier"] = repartition.get("premier") or repartition.get("1") or repartition.get("1er")
        result["allocation_deuxieme"] = repartition.get("deuxieme") or repartition.get("2") or repartition.get("2eme")
        result["allocation_troisieme"] = repartition.get("troisieme") or repartition.get("3") or repartition.get("3eme")
    ...
    if total_allocation and place_finale and place_finale <= 3:
        allocation_place = result.get(f"allocation_{['premier', 'deuxieme', 'troisieme'][place_finale-1]}")
        if allocation_place:
...
def upsert_cheval(cur, nom, date_naissance=None, sexe=None, robe=None, race=None, 
                  pays_naissance=None, entraineur=None, driver=None,
                  num_pmu=None, nom_pere=None, nom_mere=None, 
                  proprietaire=None, eleveur=None,
                  id_cheval_pmu=None, url_fiche_pmu=None):
    """
    Upsert complet d'un cheval avec gestion des doublons par nom + date_naissance.
    Enrichi avec pedigree (père, mère), infos propriétaire/éleveur et lien PMU.
    Si date_naissance est fournie, on utilise la combinaison nom+date_naissance comme clé unique.
    Sinon, on utilise juste le nom normalisé.
    """
    key = norm(nom)
    # Normaliser la race pour éviter les doublons (PUR-SANG -> PUR SANG, *ANGLO-ARABE* -> ANGLO ARABE)
    race = normalize_race(race)
    
    # Recherche par nom + date_naissance si disponible
    if date_naissance:
        cur.execute("""
            SELECT id_cheval, nom, sexe, robe, race, date_naissance, pays_naissance,
                   entraineur_courant, jockey_habituel, num_pmu, nom_pere, nom_mere,
                   proprietaire, eleveur, id_cheval_pmu, url_fiche_pmu
            FROM chevaux 
            WHERE LOWER(nom)=%s AND (date_naissance=%s OR date_naissance IS NULL)
        """, (key, date_naissance))
    else:
        cur.execute("""
            SELECT id_cheval, nom, sexe, robe, race, date_naissance, pays_naissance,
    ...
    if row:
        ...
        # Champs de base
        if sexe and row[2] != sexe:
            updates.append("sexe=%s")
            params.append(sexe)
        if robe and row[3] != robe:
            updates.append("robe=%s")
            params.append(robe)
        if race and row[4] != race:
            updates.append("race=%s")
            params.append(race)
        if date_naissance and row[5] != date_naissance:
            updates.append("date_naissance=%s")
            params.append(date_naissance)
        if pays_naissance and row[6] != pays_naissance:
            updates.append("pays_naissance=%s")
            params.append(pays_naissance)
        if entraineur and row[7] != entraineur:
            updates.append("entraineur_courant=%s")
            params.append(entraineur)
        if driver and row[8] != driver:
            updates.append("jockey_habituel=%s")
            params.append(driver)
        
        # Nouveaux champs pedigree/propriétaire
        if num_pmu and row[9] != num_pmu:
            updates.append("num_pmu=%s")
            params.append(num_pmu)
        if nom_pere and row[10] != nom_pere:
            updates.append("nom_pere=%s")
            params.append(nom_pere)
        if nom_mere and row[11] != nom_mere:
            updates.append("nom_mere=%s")
            params.append(nom_mere)
        if proprietaire and row[12] != proprietaire:
            updates.append("proprietaire=%s")
            params.append(proprietaire)
        if eleveur and row[13] != eleveur:
            updates.append("eleveur=%s")
            params.append(eleveur)
        if id_cheval_pmu and row[14] != id_cheval_pmu:
            updates.append("id_cheval_pmu=%s")
            params.append(id_cheval_pmu)
        if url_fiche_pmu and row[15] != url_fiche_pmu:
            updates.append("url_fiche_pmu=%s")
            params.append(url_fiche_pmu)
        
        if updates:
            sql = f"UPDATE chevaux SET {', '.join(updates)} WHERE id_cheval=%s"
            params.append(id_ch)
            cur.execute(sql, params)
        
        return id_ch
    else:
        # Insertion nouveau cheval avec tous les champs
        cur.execute("""
            INSERT INTO chevaux (
                nom, date_naissance, sexe, robe, race, pays_naissance,
                entraineur_courant, jockey_habituel,
                num_pmu, nom_pere, nom_mere, proprietaire, eleveur,
                id_cheval_pmu, url_fiche_pmu
            )
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            RETURNING id_cheval
        """, (nom, date_naissance, sexe, robe, race, pays_naissance, 
              entraineur, driver, num_pmu, nom_pere, nom_mere, 
              proprietaire, eleveur, id_cheval_pmu, url_fiche_pmu))
...
def upsert_cheval_counts(cur, nom, perf):
    ...
    'dernier_resultat' (musique) remplacé si plus informatif (plus long).
    'dernieres_performances' stocke les 5 dernières courses avec détails.
    """
    key = norm(nom)
    cur.execute("""SELECT id_cheval, nombre_courses_total, nombre_victoires_total,
                          nombre_courses_2025, nombre_victoires_2025, dernier_resultat
                   FROM chevaux WHERE LOWER(nom)=%s""", (key,))
    row = cur.fetchone()

    nbc  = perf.get("nbc")
    nbv  = perf.get("nbv")
    nbcY = perf.get("nbc_2025")
    nbvY = perf.get("nbv_2025")
    mus  = perf.get("musique")
    dernieres_perfs = perf.get("dernieres_perfs", [])
    
    # Convertir les dernières performances en JSON
    dernieres_perfs_json = json.dumps(dernieres_perfs, ensure_ascii=False) if dernieres_perfs else None

    if row:
        idc, nbc0, nbv0, nbcY0, nbvY0, dr0 = row
        new_dr = dr0
        if mus and (not dr0 or len(mus) > len(dr0)):
            new_dr = mus
        cur.execute("""
            UPDATE chevaux
               SET nombre_courses_total    = CASE WHEN nombre_courses_total    IS NULL THEN %s
                                                 ELSE GREATEST(nombre_courses_total,    %s) END,
                   nombre_victoires_total  = CASE WHEN nombre_victoires_total  IS NULL THEN %s
                                                 ELSE GREATEST(nombre_victoires_total,  %s) END,
                   nombre_courses_2025     = CASE WHEN nombre_courses_2025     IS NULL THEN %s
                                                 ELSE GREATEST(nombre_courses_2025,     %s) END,
                   nombre_victoires_2025   = CASE WHEN nombre_victoires_2025   IS NULL THEN %s
                                                 ELSE GREATEST(nombre_victoires_2025,   %s) END,
                   dernier_resultat        = %s,
                   dernieres_performances  = COALESCE(%s, dernieres_performances)
             WHERE id_cheval = %s
        """, (nbc, nbc, nbv, nbv, nbcY, nbcY, nbvY, nbvY, new_dr, dernieres_perfs_json, idc))
    else:
        cur.execute("""
            INSERT INTO chevaux (nom, nombre_courses_total, nombre_victoires_total,
                                 nombre_courses_2025, nombre_victoires_2025, dernier_resultat,
                                 dernieres_performances, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, CURRENT_TIMESTAMP)
        """, (key, nbc, nbv, nbcY, nbvY, mus, dernieres_perfs_json))
...
def fetch_performances(date_iso: str, reunion: int, course: int, cur, hippodrome_actuel: str, max_hist: int = 12):
    """
    ...
    for p in data.get("participants", []):
        ...
        for idx, h in enumerate(hist):
            ...
            # Compter pour 2025
            _iso, year = _parse_hist_date_any(h.get("date") or h.get("dateReunion") or h.get("jour"))
            if year == 2025:
                nb_courses_2025 += 1
                if is_win:
                    nb_victoires_2025 += 1
...
def enrich_from_course(cur, date_iso: str, reunion: int, course: int, sleep_s=None):
    ...
    if isinstance(meteo_data, dict):
        ...
        vent_kmh = meteo_data.get("forceVent") or meteo_data.get("vitesseVent") or meteo_data.get("vent")
    ...
    meteo = weather if isinstance(weather, str) else (weather.get("libelle") if isinstance(weather, dict) else None)
    ...
    for p in participants:
        ...
        url_fiche_pmu = f"https://www.pmu.fr/turf/chevaux/{id_cheval_pmu}" if id_cheval_pmu else None
        ...
        place_finale = int(place_actuelle) if place_actuelle and str(place_actuelle).isdigit() else None
        ...
        vitesse_moyenne = calculate_vitesse(distance_m, temps_sec) if distance_m and temps_sec else None
        vitesse_fin_course = vitesse_moyenne  # On peut améliorer plus tard avec données détaillées
        
        # Extraire allocations détaillées
        allocations_detail = extract_allocations_detaillees(course_data, place_finale) if place_finale else {}
        ...
        num_runners = course_data.get("nombreDeclaresPartants") or course_data.get("nombrePartants") or len(plist)
        ...
        # ========================================
        id_cheval = upsert_cheval(cur, nom, date_naissance, sexe, robe, race_cheval, 
                                   pays_origine, entraineur, driver,
        ...
        """, (
            id_cheval, nom_normalize, race_key, int(date_iso.split('-')[0]), is_win,
            reunion, course, hippodrome_actuel, venue_name,
            discipline, specialty, distance_m, start_method, rope_side, track_surface,
            total_allocation, race_name, race_conditions, race_type,
            start_time, track_condition, meteo,
            numero_dossard, driver, entraineur, proprietaire, eleveur,
            age, sexe, robe, race_cheval, pays_origine, poids,
            nom_pere, nom_mere,
            equipment, deferrage, handicap_distance,
            cote_prob_matin, cote_finale,
            place_finale, statut_arrivee, temps_str, temps_sec, reduction_km, ecarts,
            gains_course, gains_carriere, num_runners,
            is_non_runner, is_disqualified, observations,
            musique_cheval,
            pmu_reunion_id, pmu_course_id,
            rapports_json, incidents_json,
            rapport_quarte, rapport_quinte, rapport_multi, rapport_pick5, montant_enjeux_total,
        ...
        # Log enrichi
        print(f"  ✓ {nom} (#{numero_dossard or 'N/A'}, âge:{age or 'N/A'}, " +
...
def db_setup(con):
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_ccs_hippodrome ON cheval_courses_seen(hippodrome_code);")
...
def run(date_iso: str, recalc_after: bool = False, use_threading: bool = True):
    ...
    if not use_threading:
        ...
        def process_course(args):
            """Traite une course en thread-safe"""
            r, c = args
            try:
                # Chaque thread a son propre cursor
                with db_lock:
                    cur = con.cursor()
                ...
                # Rollback en cas d'erreur HTTP
                with db_lock:
                    con.rollback()
                print(f"[WARN] HTTP error on {date_iso} R{r} C{c}: {e}")
                return (r, c, f"http_error: {e}")
            except Exception as e:
                # Rollback en cas d'erreur pour éviter le blocage de la transaction
                with db_lock:
                    con.rollback()
                print(f"[WARN] Unexpected error on {date_iso} R{r} C{c}: {e}")
