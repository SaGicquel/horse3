...
class ScraperHippodromes:
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if infobox:
                    ...
                    for row in rows:
                        ...
                        if 'piste' in label or 'surface' in label:
                            if 'gazon' in value.lower() or 'herbe' in value.lower() or 'pelouse' in value.lower():
                ...
                if content:
                    ...
                    # Périmètre
                    perim_match = re.search(r'(?:périmètre|longueur).*?(\d{3,4})\s*(?:m|mètres)', text, re.IGNORECASE)
                    if perim_match:
                ...
                logger.info(f"✅ Données extraites : ville={data['ville']}, GPS={data['latitude']}/{data['longitude']}")
                return data
    ...
    def get_gps_from_nominatim(self, ville: str, nom_hippodrome: str) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            logger.warning(f"⚠️  GPS non trouvé avec nom hippodrome, essai avec ville seule")
    ...
    def enrich_hippodrome(self, id_hippodrome: int, code_pmu: str, nom: str) -> bool:
        ...
        try:
            ...
            if update_fields:
                ...
                logger.warning(f"⚠️  Aucune donnée à mettre à jour")
                cur.close()
                conn.close()
                return False
    ...
    def enrich_all_hippodromes(self, limit: Optional[int] = None):
        ...
        logger.info(f"   Hippodromes enrichis   : {enrichis} ({100*enrichis//total if total > 0 else 0}%)")
