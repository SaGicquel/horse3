...
class MarketDebiasConfig:
    ...
    gamma_grid: List[float] = field(default_factory=lambda: [0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4])
        ...
        if cluster_col and cluster_col in df.columns and self.config.gamma_by_cluster:
            ...
            for cluster, group_df in df.groupby(cluster_col):
                ...
                for race_id, race_group in group_df.groupby(race_col):
                    ...
                    date = race_group[date_col].iloc[0] if date_col in race_group.columns else race_id
...
class DynamicAlphaCalculator:
    ...
    def fit(
        ...
        def objective(params):
            ...
            alphas = self._compute_alpha_vectorized(
                df[pool_col].values,
                df[time_col].values
            )
            ...
            p_blend = self._blend_with_alphas(
                df[p_model_col].values,
                df[p_market_col].values,
                alphas
            )
    ...
    def get_alpha_batch(
        ...
        pool_sizes = df[pool_col].values if pool_col in df.columns else np.full(len(df), self.config.default_pool_size)
        minutes = df[time_col].values if time_col in df.columns else np.full(len(df), self.config.default_minutes_to_off)
...
class DynamicMarketModelBlender:
    ...
    def transform(
        ...
        p_blend_raw = self._blend_logit_with_shrinkage(
            df[p_model_col].values,
            df['p_market_debiased'].values,
            df['alpha_dynamic'].values
        )
    ...
    def fit_transform(
        ...
        """Fit puis transform."""
        self.fit(
            df, p_model_col, odds_col, race_col, label_col,
            date_col, pool_col, time_col, cluster_col
        )
        return self.transform(
            df, p_model_col, odds_col, race_col,
            pool_col, time_col, cluster_col
        )
    ...
    def evaluate(
        ...
        results['blend'] = {
            ...
            'ece': DebiasMetrics.expected_calibration_error(labels, p_blend, self.config.n_bins_ece),
        ...
        results['model_only'] = {
            ...
            'ece': DebiasMetrics.expected_calibration_error(labels, p_model, self.config.n_bins_ece),
        ...
        if p_market is not None:
            results['market_only'] = {
                ...
                'ece': DebiasMetrics.expected_calibration_error(labels, p_market, self.config.n_bins_ece),
    ...
    def print_evaluation_report(self, results: Dict[str, Any] = None):
        ...
        for source in ['blend', 'model_only', 'market_only']:
            if source in results:
                m = results[source]
                label = {'blend': 'Blend', 'model_only': 'Modèle seul', 'market_only': 'Marché corrigé'}[source]
...
def demo_usage():
    ...
    print(df_result[['race_id', 'p_model_calibrated', 'p_market_debiased', 
                     'alpha_dynamic', 'p_win_blend', 'fair_odds', 'value']].head(10))
