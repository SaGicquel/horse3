...
def main():
    ...
    logger.info(
        f"   Déjà enrichis (ville OK)  : {len(tous_hippodromes) - len(hippodromes_francais) - len(hippodromes_etrangers)}"
    )

    # Filtrer si --prioritaires-only
    if args.prioritaires_only:
        hippodromes_francais = [
            row for row in hippodromes_francais if row[1] in HIPPODROMES_PRINCIPAUX
        ]
    ...
    logger.info(
        f"   Hippodromes enrichis   : {enrichis} ({100*enrichis//(enrichis+echecs) if (enrichis+echecs) > 0 else 0}%)"
    )
