...
try:
    ...
    print(
        "⚠️ config.loader non disponible - les params ne seront pas exportés vers config/pro_betting.yaml"
    )
...
class TemperatureSoftmax:
    ...
    def _nll_loss(
        self, temperature: float, logits_by_race: List[np.ndarray], labels_by_race: List[np.ndarray]
    ) -> float:
    ...
    def transform(
        self, df: pd.DataFrame, logits_col: str = "logits_model", race_col: str = "race_id"
    ) -> np.ndarray:
        """
    ...
    def save(self, path: str):
        ...
        with open(path, "wb") as f:
            pickle.dump({"temperature": self.temperature, "config": self.config}, f)
...
class MarketModelBlender:
    ...
    def _blend_logits(self, p_model: np.ndarray, p_market: np.ndarray, alpha: float) -> np.ndarray:
        """
    ...
    def _cv_score(
        ...
        for i in range(self.config.alpha_cv_folds):
            ...
            p_blend = self._blend_logits(p_model[val_mask], p_market[val_mask], alpha)
    ...
    def fit(
        ...
        if cluster_col and cluster_col in df.columns:
            for cluster, group in df.groupby(cluster_col):
                ...
                dates_c = (
                    group[date_col].values if date_col in group.columns else np.arange(len(group))
                )
    ...
    def transform(
        ...
        if cluster_col and cluster_col in df.columns and self.alpha_by_cluster:
            for cluster, group in df.groupby(cluster_col):
                ...
                p_blend[idx] = self._blend_logits(p_model[idx], p_market[idx], alpha)
        else:
    ...
    def save(self, path: str):
        with open(path, "wb") as f:
            pickle.dump({"alpha": self.alpha, "alpha_by_cluster": self.alpha_by_cluster}, f)
...
class CalibrationPipeline:
    ...
    def __init__(self, config: CalibrationConfig = None):
        ...
        self.dynamic_blender: Optional["DynamicMarketModelBlender"] = (
            None  # Nouveau blender dynamique
        )
    ...
    def _prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "odds_market_preoff" in df.columns and "p_market" not in df.columns:
            ...
            df["p_market"] = df.groupby("race_id")["p_market"].transform(lambda x: x / x.sum())

        return df

    def _select_best_calibrator(
        self, val_df: pd.DataFrame, p_col: str = "p_model_norm", label_col: str = "label_win"
    ) -> str:
        ...
        metrics_platt = {
            ...
            "ece": CalibrationMetrics.expected_calibration_error(
                labels, p_platt, self.config.n_bins_ece
            )[0],
        }
        ...
        metrics_isotonic = {
            ...
            "ece": CalibrationMetrics.expected_calibration_error(
                labels, p_isotonic, self.config.n_bins_ece
            )[0],
        }

        print("\n📈 Comparaison calibrateurs (validation):")
        print(
            f"   Platt    - Brier: {metrics_platt['brier']:.4f}, ECE: {metrics_platt['ece']:.4f}, NLL: {metrics_platt['nll']:.4f}"
        )
        print(
            f"   Isotonic - Brier: {metrics_isotonic['brier']:.4f}, ECE: {metrics_isotonic['ece']:.4f}, NLL: {metrics_isotonic['nll']:.4f}"
        )
    ...
    def fit(
        ...
        if len(train_df) < self.config.min_train_samples:
            raise ValueError(
                f"Pas assez de données d'entraînement: {len(train_df)} < {self.config.min_train_samples}"
            )

        # 1. Normalisation softmax avec température
        print("\n📐 Étape 1: Normalisation Softmax")
        self.scaler = TemperatureSoftmax(self.config)
        train_df["p_model_norm"] = self.scaler.fit_transform(
            train_df, logits_col, race_col, label_win_col
        )
        ...
        self.calibrator_platt.fit(train_df["p_model_norm"].values, train_df[label_win_col].values)

        # Fit Isotonic
        self.calibrator_isotonic = IsotonicCalibrator()
        self.calibrator_isotonic.fit(
            train_df["p_model_norm"].values, train_df[label_win_col].values
        )

        # Sélectionner le meilleur
        self.best_calibrator_type = self._select_best_calibrator(
            val_df, "p_model_norm", label_win_col
        )

        # Appliquer calibration
        calibrator = (
            self.calibrator_isotonic
            if self.best_calibrator_type == "isotonic"
            else self.calibrator_platt
        )
        ...
        use_dynamic = (
            DEBIASER_AVAILABLE
            and (self.config.use_gamma_correction or self.config.use_dynamic_alpha)
            and "p_market" in train_df.columns
        )
        ...
        if use_dynamic:
            ...
            debias_results = self.dynamic_blender.evaluate(
                test_df, p_model_col="p_calibrated", odds_col=odds_col, label_col=label_win_col
            )
    ...
    def _evaluate(self, test_df: pd.DataFrame, p_col: str, label_col: str, odds_col: str):
        ...
        if odds_col in test_df.columns:
            ...
            # Flat betting
            flat_results = simulator.simulate_flat_betting(test_df, p_col, odds_col, label_col)
            self.metrics["profit_flat"] = flat_results
            print("\n   📈 Simulation Flat Betting:")
            print(f"      Paris: {flat_results['n_bets']}, ROI: {flat_results['roi']:.1f}%")

            # Kelly betting
            kelly_results = simulator.simulate_kelly_betting(test_df, p_col, odds_col, label_col)
            self.metrics["profit_kelly"] = kelly_results
            print("   📈 Simulation Kelly:")
            print(
                f"      Paris: {kelly_results['n_bets']}, ROI: {kelly_results['roi']:.1f}%, Drawdown Max: {kelly_results['max_drawdown_pct']:.1f}%"
            )
        ...
        self.metrics["stats"] = {
            ...
            "gamma_by_cluster": self.dynamic_blender.gamma_corrector.gamma_by_cluster
    ...
    def _export_to_config(self):
        ...
        try:
            ...
            if self.dynamic_blender:
                ...
                print(
                    f"   - alpha_coefficients: a0={self.dynamic_blender.alpha_calculator.a0:.3f}, "
    ...
    def transform(
        ...
        if self.dynamic_blender and odds_col in df.columns:
            ...
            df["p_final"] = self.blender.transform(
                df, p_model_col="p_calibrated", p_market_col="p_market", cluster_col=cluster_col
            )
        else:
    ...
    def load(cls, artifacts_dir: str, report_name: str = None) -> "CalibrationPipeline":
        ...
        if report_name is None:
            reports = sorted(
                [f for f in os.listdir(artifacts_dir) if f.startswith("calibration_report")]
            )
            if not reports:
        ...
        if "dynamic_blender" in artifacts and DEBIASER_AVAILABLE:
            try:
                pipeline.dynamic_blender = DynamicMarketModelBlender.load(
                    artifacts["dynamic_blender"]
                )
            except Exception as e:
...
def generate_synthetic_data(
    n_races: int = 500, horses_per_race: int = 12, seed: int = 42
) -> pd.DataFrame:
    """
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="calibration_artifacts", help="Dossier de sortie"
    )
    ...
    if args.test or args.data is None:
        ...
        # Configuration
        config = CalibrationConfig(
            artifacts_dir=args.output, temperature_init=1.5, min_samples_cluster=100
        )
        ...
        result_df = loaded_pipeline.transform(
            new_df, logits_col="logits_model", race_col="race_id", cluster_col="discipline"
        )

        print(f"\n✅ Transformation sur nouvelles données: {len(result_df)} lignes")
        print("   Colonnes ajoutées: p_model_norm, p_calibrated, p_final")

        # Vérification normalisation
        sums = result_df.groupby("race_id")["p_final"].sum()
        print(
            f"   Somme p_final par course: min={sums.min():.4f}, max={sums.max():.4f}, mean={sums.mean():.4f}"
        )
