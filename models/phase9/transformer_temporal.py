...
class TemporalTransformer(nn.Module):
    ...
    """
    def __init__(self, input_dim: int, d_model: int, n_head: int, n_layers: int, dropout: float = 0.1, sequence_length: int = 10):
        """
        Args:
        ...
        encoder_layers = nn.TransformerEncoderLayer(d_model, n_head, dim_feedforward=d_model*4, dropout=dropout, batch_first=True)
    ...
    def forward(self, src: torch.Tensor, src_key_padding_mask: torch.Tensor) -> torch.Tensor:
        ...
        transformer_output = self.transformer_encoder(pos_encoded_src, src_key_padding_mask=src_key_padding_mask)
...
if __name__ == '__main__':
    ...
    # Vérifications
    assert output_logits.shape == (BATCH_SIZE, 1), f"La shape de sortie est incorrecte: {output_logits.shape}"
