    ...
    try:
        ...
        if min_date and max_date:
            ...
            try:
                ...
                if jours_retard == 0:
                    print(f"✅ Base à jour (dernière course: aujourd'hui)")
                elif jours_retard == 1:
                    print(f"⚠️  Base avec 1 jour de retard (dernière course: hier)")
                elif jours_retard <= 7:
        ...
        if top_chevaux:
            for i, (nom, total, victoires) in enumerate(top_chevaux, 1):
                ...
                print(f"  {i:2d}. {nom:30s} {total:3d} courses, {victoires:2d} victoires ({taux:4.1f}%)")
        else:
            print("  Aucune donnée")
