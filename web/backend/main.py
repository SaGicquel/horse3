...
DEFAULT_CONFIG = {
    ...
    "kelly": {"fraction": 0.25, "value_cutoff": 0.0, "max_stake_pct": 0.05},
    "portfolio": {
...
def get_user_from_request(authorization: str | None = Header(None)) -> dict[str, Any] | None:
    ...
    try:
        ...
        if row:
            return UserSettings(
                ...
                profil_risque=row[1],
                kelly_profile=row[2],
                custom_kelly_fraction=row[3],
                value_cutoff=row[4],
                cap_per_bet=row[5],
                daily_budget_rate=row[6],
                max_unit_bets_per_race=row[7],
                rounding_increment_eur=row[8],
                per_ticket_rate=row[9],
                max_pack_rate=row[10],
                market_mode=row[11],
                takeout_rate=row[12],
            )
        else:
        ...
        if USE_POSTGRESQL:
            ...
            """,
                (
                    user_id,
                    settings.bankroll,
                    settings.profil_risque,
                    settings.kelly_profile,
                    settings.custom_kelly_fraction,
                    settings.value_cutoff,
                    settings.cap_per_bet,
                    settings.daily_budget_rate,
                    settings.max_unit_bets_per_race,
                    settings.rounding_increment_eur,
                    settings.per_ticket_rate,
                    settings.max_pack_rate,
                    settings.market_mode,
                    settings.takeout_rate,
                ),
            )
        else:
...
def calculate_personalized_stake(runner: dict, user_params: UserBettingParams) -> dict[str, Any]:
    ...
    if value_pct < config["min_value_pct"]:
        return {
            "eligible": False,
            "reason": f"Value {value_pct:.1f}% < seuil {config['min_value_pct']:.1f}%",
        }
...
try:
    ...
    USE_POSTGRESQL = True
    print("[INFO] Utilisation de PostgreSQL (module local)")
except ImportError:
    ...
    try:
        ...
        USE_POSTGRESQL = True
        print("[INFO] Utilisation de PostgreSQL (module parent)")
...
class Settings(BaseModel):
    ...
    kelly: SettingsKelly
    budget: SettingsBudget | None = None
    betting_defaults: SettingsBettingDefaults | None = None
    exotics_defaults: SettingsExoticsDefaults | None = None
...
class BetUpdateRequest(BaseModel):
    ...
    status: Literal["PENDING", "WIN", "LOSE", "VOID"] | None = None
    notes: str | None = None
...
def normalize_bet_status(status: str | None) -> str:
...
def infer_event_date(race_key: str | None, provided: date | None) -> date | None:
...
def hash_password(password: str, salt: bytes | None = None) -> (str, str):
...
def get_user_from_token(token: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        return {"id": row[0], "email": row[1], "display_name": row[2], "created_at": created_at}
    finally:
        con.close()
...
def build_monitoring_from_bets(bets: list[dict[str, Any]]) -> MonitoringStats:
    ...
    resolved_stake = sum(
        to_float(b["stake"]) for b in bets if b["status"] in ("WIN", "LOSE", "VOID")
    )
    ...
    recent_bets = sorted(
        bets, key=lambda b: b.get("created_at") or datetime.utcnow(), reverse=True
    )[:50]
...
def get_rapport_for_bet_type(
    rapports: dict, bet_type: str, selection: str, num_pmu: str = None
) -> float:
    """
    ...
    # SIMPLE GAGNANT
    if (
        "GAGNANT" in bet_type_upper
        and "PLACÉ" not in bet_type_upper
        and "E/P" not in bet_type_upper
    ):
    ...
    # SIMPLE PLACÉ
    if (
        "PLACÉ" in bet_type_upper
        and "GAGNANT" not in bet_type_upper
        and "E/P" not in bet_type_upper
        and "COUPLÉ" not in bet_type_upper
    ):
    ...
    if "TIERC" in bet_type_upper:
        ...
        if len(nums) >= 3 and "TIERCE" in rapports:
            ...
            if "ORDRE" in bet_type_upper:
                ...
                return rapports["TIERCE"]["desordre"].get(combo) or rapports["TIERCE"]["ordre"].get(
                    combo
                )
    ...
    if "QUART" in bet_type_upper:
        ...
        if len(nums) >= 4 and "QUARTE_PLUS" in rapports:
            ...
            if "ORDRE" in bet_type_upper:
                ...
                return rapports["QUARTE_PLUS"]["desordre"].get(combo) or rapports["QUARTE_PLUS"][
                    "ordre"
                ].get(combo)
    ...
    if "QUINT" in bet_type_upper:
        ...
        if len(nums) >= 5 and "QUINTE_PLUS" in rapports:
            ...
            if "ORDRE" in bet_type_upper:
                ...
                return rapports["QUINTE_PLUS"]["desordre"].get(combo) or rapports["QUINTE_PLUS"][
                    "ordre"
                ].get(combo)
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                nb_partants = sum(
                    ...
                    if not p.get("nonPartant", False) and p.get("statut") != "NON_PARTANT"
                )
                ...
                for p in participants:
                    ...
                    is_non_partant = (
                        p.get("nonPartant", False)
                        or p.get("statut") == "NON_PARTANT"
                        or p.get("incident") == "NON_PARTANT"
                    )
...
def update_race_results_in_db(cur, race_key: str, race_data: dict):
    ...
    print(
        f"[DEBUG] update_race_results_in_db: {updated}/{total} chevaux mis à jour pour {race_key}"
    )
    return updated > 0
    ...
    if cur and con:
        ...
        if success:
            try:
                con.commit()
                print(
                    f"[INFO] Résultats mis à jour pour {race_key} ({race_data.get('nb_partants', 0)} partants)"
                )
            except Exception as e:
...
def determine_bet_result(
    ...
    # ═══════════════════════════════════════════════════════════════════
    if (
        ("GAGNANT" in bet_type_upper or "E_GAGNANT" in bet_type_upper)
        and "PLACÉ" not in bet_type_upper
        and "E/P" not in bet_type_upper
        and "COUPLÉ" not in bet_type_upper
        and "COUPLE" not in bet_type_upper
    ):
    ...
    if "COUPLÉ ORDRE" in bet_type_upper or "COUPLE ORDRE" in bet_type_upper:
        ...
        if (
            norm(top_arrivee[0]) == selections_norm[0]
            and norm(top_arrivee[1]) == selections_norm[1]
        ):
            return "WIN"
        return "LOSE"
    ...
    if "TRIO" in bet_type_upper or "TIERCÉ" in bet_type_upper or "TIERCE" in bet_type_upper:
        ...
        selections_norm = (
            [norm(s) for s in selections[:3]]
            if len(selections) >= 3
            else [norm(s) for s in selections]
        )
...
async def login_user(payload: LoginRequest):
    ...
    try:
        ...
        return AuthResponse(
            ...
            expires_at=expires_at,
            user=UserProfile(
                id=row[0], email=row[1], display_name=row[4], created_at=serialize_datetime(row[5])
            ),
        )
    except HTTPException:
        con.rollback()
...
async def create_bet(payload: BetCreateRequest, authorization: str | None = Header(None)):
...
async def list_bets(authorization: str | None = Header(None)):
...
async def update_bet(
    bet_id: int, payload: BetUpdateRequest, authorization: str | None = Header(None)
):
...
async def delete_bet(bet_id: int, authorization: str | None = Header(None)):
...
async def refresh_bet_result(bet_id: int, authorization: str | None = Header(None)):
    """
    ...
    try:
        ...
        if race_data and "participants" in race_data:
            ...
            if horse_data:
                ...
                print(
                    f"[DEBUG] Cheval {selection_norm} non trouvé dans les {len(participants)} participants"
                )
        else:
        ...
        if rapports_definitifs:
            ...
            if rapport:
                ...
                print(
                    f"[INFO] Pas de rapport pour {bet_type}, utilisation cote simple gagnant: {updated_odds}"
                )
        elif cote_finale:
        ...
        if place_finale is not None and not already_resolved:
            ...
            if updated_status in ("WIN", "LOSE"):
                ...
                con.commit()
                print(
                    f"[INFO] Pari #{bet_id} ({selection}): {updated_status} (place: {place_finale}, cote: {updated_odds}, partants: {nb_partants}, type: {bet_type})"
                )
            ...
            if race_data and race_data.get("course_terminee"):
                ...
                print(
                    f"[INFO] Pas de résultats disponibles pour {race_key} - {selection}, statut reste PENDING"
                )
...
async def get_bets_summary(authorization: str | None = Header(None)):
    ...
    try:
        ...
        history = [{"date": d, "pnl": round(p, 2)} for d, p in sorted(history_map.items())]
...
async def get_settings():
    ...
    try:
        ...
        betting_defaults = SettingsBettingDefaults(
            ...
            kelly_fraction_map=bet_def.get(
                "kelly_fraction_map", {"SUR": 0.25, "STANDARD": 0.33, "AMBITIEUX": 0.50}
            ),
            custom_kelly_fraction=bet_def.get("custom_kelly_fraction", 0.33),
            value_cutoff=bet_def.get(
        ...
        return Settings(
            ...
            budget=SettingsBudget(daily_limit=config.get("budget", {}).get("daily_limit", 100.0)),
...
async def update_settings(settings: Settings):
    ...
    try:
        ...
        if "markets" not in current_config:
            current_config["markets"] = {}
        ...
        if "kelly" not in current_config:
            current_config["kelly"] = {}
        ...
        if settings.budget:
            if "budget" not in current_config:
        ...
        if settings.betting_defaults:
            if "betting_defaults" not in current_config:
                current_config["betting_defaults"] = {}
        ...
        if settings.exotics_defaults:
            if "exotics_defaults" not in current_config:
                current_config["exotics_defaults"] = {}
            current_config["exotics_defaults"]["per_ticket_rate"] = (
                settings.exotics_defaults.per_ticket_rate
            )
            current_config["exotics_defaults"]["max_pack_rate"] = (
                settings.exotics_defaults.max_pack_rate
            )
...
async def get_user_settings_endpoint(user: dict | None = Depends(get_user_from_request)):
    """
...
async def update_user_settings_endpoint(
    settings: UserSettings, user: dict | None = Depends(get_user_from_request)
):
    """
...
async def get_monitoring_stats(authorization: str | None = Header(None)):
    """
    ...
    try:
        ...
        if finished_bets > 0:
            ...
            daily_pnl = (
                df_finished.groupby(df_finished["date"].dt.strftime("%Y-%m-%d"))["pnl_cumul"]
                .last()
                .reset_index()
            )
    ...
    try:
        ...
        for race, taux, nb in races_top:
            if race and taux:
                ...
                variables_predictives.append(
                    {"nom": f"Race: {race[:20]}", "importance": importance}
                )
        ...
        if sexes_top:
            ...
            if sexe and taux:
                ...
                variables_predictives.append(
                    {"nom": f"Performance {sexe}", "importance": importance}
                )
        ...
        while len(variables_predictives) < 5:
            if len(variables_predictives) == 0:
                variables_predictives.append(
                    {"nom": "Nombre de courses", "importance": min(90, round(total_courses / 50))}
                )
            elif len(variables_predictives) == 1:
                variables_predictives.append(
                    {
                        "nom": "Expérience cheval",
                        "importance": min(85, round(taux_moyen_chevaux * 5)),
                    }
                )
            elif len(variables_predictives) == 2:
                variables_predictives.append(
                    {
                        "nom": "Historique victoires",
                        "importance": min(80, round((total_victoires / total_courses * 100) * 0.8)),
                    }
                )
            elif len(variables_predictives) == 3:
                variables_predictives.append(
                    {
                        "nom": "Professionnels actifs",
                        "importance": min(75, round((nb_entraineurs + nb_jockeys) / 10)),
                    }
                )
            else:
...
async def get_chevaux(
    ...
    try:
        ...
        con.close()
        return {"chevaux": results, "total": total_count}
        ...
        return {
            ...
            "statistiques": {
                ...
                "2025": {
                    ...
                    "taux": round(
                        (nb_victoires_2025 / nb_courses_2025 * 100) if nb_courses_2025 else 0, 1
                    ),
                },
            },
    ...
    try:
        ...
        if USE_POSTGRESQL:
            ...
            for row in cur.fetchall():
                ...
                top_performers.append(
                    {"nom": nom, "courses": courses, "victoires": victoires, "taux": round(taux, 1)}
                )
            ...
            for row in rows:
                ...
                stats_sexe.append(
                    {"sexe": sexe, "total": total, "taux_moyen": round(taux_moyen, 1)}
                )
...
async def get_analytics_jockeys():
    ...
    try:
        ...
        for row in cur.fetchall():
            ...
            taux_moyen = float(row[2]) if row[2] is not None else 0.0
            top_jockeys.append(
                {"nom": jockey, "nb_chevaux": nb_chevaux, "taux_reussite": round(taux_moyen, 1)}
            )

        con.close()

        return {"top_jockeys": top_jockeys}
    ...
    try:
        ...
        for row in cur.fetchall():
            ...
            top_entraineurs.append(
                {"nom": entraineur, "nb_chevaux": nb_chevaux, "taux_reussite": round(taux_moyen, 1)}
            )

        con.close()

        return {"top_entraineurs": top_entraineurs}
                ...
                hippodromes.append(
                    {
                        ...
                        "taux_victoire": round(
                            (nb_victoires / nb_courses * 100) if nb_courses > 0 else 0, 1
                        ),
                    }
                )
...
async def get_analytics_age():
    ...
    try:
        ...
        for row in cur.fetchall():
            ...
            stats.append(
                {"age": f"{age} ans", "total": total, "victoires": victoires, "taux": taux}
            )
...
def run_benter_head_for_date(search_date, cur=None, tau=None, min_support=60):
    ...
    try:
        ...
        track_effects = _load_effects(
            cur, "COALESCE(hippodrome_code, hippodrome_nom)", alias="hippodrome_key"
        )
        ...
        for (
            ...
            context_effect = (
                ...
                + _compute_effect(
                    etat_piste, going_effects, base_logit, global_rate, min_support, prior_strength
                )
                + _compute_effect(
                    distance_bucket,
                    distance_effects,
                ...
                + _compute_effect(
                    corde, corde_effects, base_logit, global_rate, min_support, prior_strength
                )
            )
...
async def get_betting_recommendations(
    ...
    try:
        ...
        recommendations.sort(
            key=lambda x: (niveau_order.get(x["recommendation"]["niveau"], 99), -x["value_bet"])
        )
        ...
        stats = {
            ...
            "paris_forts": len(
                [r for r in recommendations if r["recommendation"]["niveau"] == "FORT"]
            ),
            "paris_moyens": len(
                [r for r in recommendations if r["recommendation"]["niveau"] == "MOYEN"]
            ),
            "paris_faibles": len(
                [r for r in recommendations if r["recommendation"]["niveau"] == "FAIBLE"]
            ),
            "value_moyenne": round(
                sum(r["value_bet"] for r in recommendations) / len(recommendations), 1
            )
            if recommendations
            else 0,
            "score_moyen": round(sum(r["score"] for r in recommendations) / len(recommendations), 1)
            if recommendations
            else 0,
        }
...
async def get_cheval_profile(nom: str):
    ...
    try:
        ...
        recent_win_rate = round(
            (recent_wins / recent_courses * 100) if recent_courses > 0 else 0, 1
        )
...
async def get_hippodromes():
    ...
    try:
        ...
        hippodromes = []
        for row in cur.fetchall():
            hippodromes.append({"nom": row[0], "courses": row[1]})
...
def format_recommendation_from_generator(
    runner, race_key, race_info, user_params: UserBettingParams | None = None
):
    """
    ...
    try:
        ...
        # Filtrer seulement les chevaux avec value trop négative (< -5%)
        if value_pct < -5:
            return None
        ...
        if user_params:
            ...
            # Si kelly_stake absent, calculer un kelly basique
            if kelly_stake == 0 and kelly_pct == 0 and p_win > 0 and odds > 1:
                # Kelly brut = (b*p - q) / b où b = odds-1, p = p_win, q = 1-p_win
                b = odds - 1
                kelly_raw = (b * p_win - (1 - p_win)) / b
                kelly_pct = max(0, kelly_raw * 25)  # 1/4 Kelly en %
                kelly_stake = kelly_pct / 100  # en fraction

            stake_euros = kelly_stake * 1000
            stake_pct = kelly_pct

            # Mise minimum de 5€ pour éviter les mises ridicules
            if stake_euros > 0 and stake_euros < 5:
                stake_euros = 5.0
            # Plafond à 5% de la bankroll (50€ pour 1000€)
            if stake_euros > 50:
                stake_euros = 50.0
...
async def get_paris_recommandations_jour(user: dict | None = Depends(get_user_from_request)):
    """
    ...
    if not user:
        raise HTTPException(
            status_code=401,
            detail="Authentification requise pour les recommandations personnalisées",
        )
    ...
    try:
        ...
        if not courses:
            ...
            return {
                ...
                "resume": {
                    "total_paris_sur": 0,
                    "total_paris_equilibre": 0,
                    "total_paris_risque": 0,
                },
                "user_profile": {
                    "bankroll": user_settings.bankroll,
                    "profil_risque": user_settings.profil_risque,
                },
            }
                ...
                if generator:
                    ...
                    if pronostic_dict and "runners" in pronostic_dict:
                        ...
                        for runner in pronostic_dict["runners"]:
                            rec = format_recommendation_from_generator(
                                runner, race_key, race_info, user_params
                            )
                            if rec:
                                ...
                                if (
                                    ...
                                    <= user_settings.bankroll * config["max_daily_pct"] / 100
                                ):
        ...
        for key in all_recommandations:
            all_recommandations[key].sort(
                key=lambda x: (x.get("esperance", 0), x.get("mise_recommandee_num", 0)),
                reverse=True,
            )
        ...
        return {
            ...
            "profil_utilisateur": {
                ...
                "budget_restant": round(
                    user_settings.bankroll * config["max_daily_pct"] / 100 - total_mise_jour, 1
                ),
                "pourcentage_bankroll_utilise": round(
                    (total_mise_jour / user_settings.bankroll) * 100, 1
                ),
            },
            "meilleurs_paris": {
        ...
        "profils": {
            profil.value: {
                **ConfigProfil.get_config(profil),
                ...
                "emoji": {"PRUDENT": "🛡️", "STANDARD": "⚖️", "AGRESSIF": "🚀"}[profil.value],
            }
...
def analyser_course_pour_paris(cur, race_key: str) -> dict:
    ...
    for p in participants:
        if p["cote"] and p["cote"] < 5 and p["score_global"] >= 60:
            ...
            if p["proba_victoire"] >= 25:
                ...
                nb_partants >= 8
                and len([x for x in participants[:3] if x["score_global"] >= 50]) >= 2
            ):
    ...
    for p in participants:
        if (
            p["cote"]
            and p["cote"] > 15
            and p["score_global"] >= 40
            and len(p.get("signaux_positifs", [])) >= 2
        ):
...
async def get_picks_today(zone: str = None, bankroll: float = None, simulation: bool = False):
    ...
    try:
        ...
        if not rows:
            ...
            try:
                use_place_model = os.getenv("USE_PLACE_MODEL", "1").lower() in (
                    "1",
                    "true",
                    "yes",
                    "on",
                ...
                if use_place_model:
                    ...
                    pm = load_place_model()
                    place_model_meta = (
                        {"status": "ok", **(pm.metadata or {})} if pm else {"status": "missing"}
                    )
            except Exception as e:
        ...
        for row in rows:
            ...
            if statut_norm not in allowed_status or any(
                key in statut_norm for key in EXCLUDED_STATUS_KEYWORDS
            ):
                skipped_status += 1
                continue
            ...
            score = calculate_prediction_score(
                cote, cote_ref, tendance, amplitude, est_favori, avis
            )
            ...
            if benter_runner:
                p_model = (
                    benter_runner.get("p_calibrated") or benter_runner.get("p_model_norm") or 0
                )
        ...
        micro_neg_threshold = float(
            os.getenv("MICRO_DRIFT_NEG_THRESHOLD", "0.10")
        )  # 10% log-odds défavorable → hold
        micro_scale_threshold = float(
            os.getenv("MICRO_DRIFT_SCALE_THRESHOLD", "0.05")
        )  # 5% → scale-down
        ...
        if use_champion:
            try:
                from services.champion_predictor import (
                    ChampionPredictor,
                    default_champion_artifacts,
                )

                predictor = ChampionPredictor(default_champion_artifacts())

                def _build_features_for_race(
                    runners: list[dict[str, Any]], race_date: str
                ) -> list[dict[str, Any]]:
                    ...
                    mean_forme = 0.0
                    vals = [
                        float((sc.get(r.get("id_cheval"), (0, 0, 0, 0, 0))[0]) or 0)
                        for r in runners
                    ]
                    if vals:
                    ...
                    for idx, r in enumerate(runners):
                        cid = r.get("id_cheval")
                        forme_5c, apt_dist, apt_piste, apt_hippo, regularite = sc.get(
                            cid, (0, 0, 0, 0, 0)
                        )
                        nb_courses_12m, nb_victoires_12m, nb_places_12m, recence = hist.get(
                            cid, (0, 0, 0, 90.0)
                        )
                        ...
                        # One-hot état de piste (correspond aux 5 features du modèle champion)
                        etat_features = {
                            ...
                            "etat_Leger": 1.0
                            if "léger" in etat.lower() or "leger" in etat.lower()
                            else 0.0,
                        }
                        ...
                        interaction_aptitude_distance = float(
                            (apt_dist or 0) * (distance_norm or 0)
                        )
                        interaction_elo_niveau = float((elo_cheval / 1000.0) * (mean_forme or 0))
                        interaction_synergie_forme = float(
                            (synergie_j + synergie_t) * (forme_5c or 0)
                        )
                        interaction_aptitude_popularite = float(
                            (apt_dist or 0) * (1.0 / max(1.0, float(rank_map.get(idx, 1))))
                        )
                        interaction_regularite_volume = float(
                            (regularite or 0) * (nb_courses_12m or 0)
                        )
                        ...
                        feature_rows.append(
                            {
                                # Features attendues (correspond exactement à feature_names.json)
                                ...
                                "meteo_code": 0.0,  # String code, toujours 0
                                # Interactions
                ...
                for rk, runners in race_groups.items():
                    ...
                    if not feat_rows:
                        continue
                    # Nettoyer les valeurs infinies/nan dans les features avant predict
                    for fr in feat_rows:
                        for k, v in fr.items():
                            try:
                                fv = float(v)
                                if not (-1e10 < fv < 1e10) or fv != fv:  # inf or nan
                                    fr[k] = 0.0
                            except (ValueError, TypeError):
                                fr[k] = 0.0
        ...
        try:
            for race_key, runners in race_groups.items():
                ...
                alpha_raw = (
                    alpha_base
                    + alpha_pool_coef * math.log(max(pool_size, 1))
                    - alpha_time_coef * minutes_to_off
                )
                ...
                if place_model and p_place_race:
                    try:
                        ...
                        sorted_idx = sorted(
                            range(len(cotes_r)), key=lambda i: (cotes_r[i] or 999999)
                        )
                        rank_map = {idx: rank + 1 for rank, idx in enumerate(sorted_idx)}
                        pm_inputs = []
                        ...
                        p_place_race = [
                            max(0.0, min(0.999, float(x) * target_sum / s_adj)) for x in p_place_adj
                        ]
                    except Exception:
                        pass
                ...
                for idx, r in enumerate(runners):
                    ...
                    if r["tendance"] in ("+", "-") and r["amplitude"] is not None:
                        sign = (
                            1 if r["tendance"] == "+" else -1
                        )  # + = dérive cote vers le haut (défavorable)
                    ...
                    if drift_pct >= micro_neg_threshold:
                        micro_action = "hold"
                        micro_reason = (
                            f"drift défavorable {drift_pct:.2f}≥{micro_neg_threshold:.2f}"
                        )
                    elif drift_pct >= micro_scale_threshold:
                        micro_action = "scale_down"
                        micro_reason = (
                            f"drift défavorable modéré {drift_pct:.2f}≥{micro_scale_threshold:.2f}"
                        )
                    ...
                    if p_win >= 0.15 and p_place >= 0.45:
                        ...
                        if ev_ep > 0:
                            ep_risk = (
                                "Faible"
                                if (p_place >= 0.60 and (cote and cote <= 10))
                                else "Modéré"
                            )
                            bet_types.append(
                                {
                                    ...
                                    "risk": ep_risk,
                                    "proba": round(p_place * 100, 1),
                                    "cote_estimee": round((cote + cote_place) / 2, 2)
                                    if cote
                                    else round(cote_place, 2),
                                    ...
                                    "description": f"Gagnant ({cote:.2f}) + Placé (~{cote_place:.2f} estimée)"
                                    if cote
                                    else "Gagnant + Placé",
                                }
                            )
                    if p_win >= 0.30 and value_pct >= 10:
                        win_risk = (
                            "Faible"
                            if (cote and cote <= 4 and p_win >= 0.22)
                            else ("Modéré" if (cote and cote <= 8 and p_win >= 0.15) else "Élevé")
                        )
                        bet_types.append(
                            {
                                "type": "SIMPLE GAGNANT",
                        ...
                        bet_types.append(
                            {
                                ...
                                "value": round(value_pct, 1),
                                "kelly": round(kelly_pct, 1),
                                "description": f"Value bet (+{value_pct:.0f}% value - cote {cote:.2f})"
                                if cote
                                else "Value bet",
                            }
                        )
                        ...
                        bet_types.append(
                            {
                                ...
                                "value": round(value_pct, 1),
                                "kelly": round(kelly_pct, 1),
                                "description": f"Outsider value (+{value_pct:.0f}% - petite mise conseillée)"
                                if cote
                                else "Outsider value",
                            }
                        )
                        ...
                        if value_pct > 0 and p_win >= 0.10:
                            win_risk = (
                                ...
                                if (cote and cote <= 4 and p_win >= 0.22)
                                    ...
                                    "Modéré" if (cote and cote <= 8 and p_win >= 0.15) else "Élevé"
                                )
                            )
                            bet_types.append(
                                {
                                    ...
                                    "value": round(value_pct, 1),
                                    "kelly": round(kelly_pct, 1),
                                    "description": f"Value positive (+{value_pct:.0f}%) - mise prudente"
                                    if cote
                                    else "Value positive",
                                }
                            )
                    ...
                    picks.append(
                        {
                            ...
                            "rationale": generate_rationale(
                                r["score"], value_pct, r["tendance"], r["est_favori"], r["avis"]
                            ),
                            "historique": {
                            ...
                            "resultat": "Gagné"
                            if r["is_win"] == 1
                            else ("Placé" if r["place"] and r["place"] <= 3 else None),
                            "analysis_source": analysis_source_label,
                            "p_benter_model": round(r["p_benter_raw"], 4)
                            if r["p_benter_raw"] is not None
                            else None,
            ...
            analyses.append(
                {
                    ...
                    "resultat": "Gagné"
                    if is_win == 1
                    else ("Placé" if place and place <= 3 else None),
                }
            )
...
async def build_exotics(request: ExoticsBuildRequest):
    ...
    try:
        ...
        use_advanced = (
            request.use_advanced_estimator and PLACE_ESTIMATOR_LOADED and ESTIMATOR_AVAILABLE
        )
        ...
        if use_advanced:
            try:
                ...
                analysis = service.analyze_race(
                    partants=partants, discipline=discipline, n_simulations=request.n_simulations
                )

                # Générer les packs de tickets
                packs = service.generate_packs(analysis=analysis, budget=budget, pack_type=pack)
            ...
            tickets.append(
                {
                    ...
                    "ev": round(
                        sum(p["score"] for p in base[:3]) / 3 * 0.1 - budget * 0.4 * 0.15, 2
                    ),
                    "couverture": 0.6,
            ...
            tickets.append(
                {
                    ...
                    "ev": round(
                        sum(p["score"] for p in base[:4]) / 4 * 0.15 - budget * 0.35 * 0.1, 2
                    ),
                    "couverture": 0.5,
...
async def analyze_exotics_advanced(request: ExoticAdvancedRequest):
    ...
    if not PLACE_ESTIMATOR_LOADED or not ESTIMATOR_AVAILABLE:
        raise HTTPException(status_code=503, detail="Service d'estimation avancé non disponible")
    ...
    try:
        ...
        top_combos = [
            c
            for c in analysis.get("top_combos", [])
            ...
            p_place_detail.append(
                {
                    ...
                    "nom": p["nom"],
                    "cote": p["cote"],
                    "p_win": analysis.get("p_win", [])[i]
                    if i < len(analysis.get("p_win", []))
                    else 0,
                    "p_place_2": analysis.get("p_place_2", [])[i]
                    if i < len(analysis.get("p_place_2", []))
                    else 0,
                    "p_place_3": analysis.get("p_place_3", [])[i]
                    if i < len(analysis.get("p_place_3", []))
                    else 0,
                    "score_model": p["score"],
                }
            )
...
def _generate_exotic_recommendations(analysis: dict, partants: list) -> list:
    ...
    if p_win:
        ...
        if max(p_win) > 0.35:
            ...
            recommendations.append(
                f"⚠️ Favori dominant ({fav_nom}, p={max(p_win):.1%}) - Trio ordre recommandé"
            )
    ...
    if ev_positive:
        ...
        recommendations.append(
            "⚡ Aucune combinaison à forte EV - Mises conservatrices recommandées"
        )
...
def _compute_engine_portfolio(
    ...
    for row in rows:
        (nom, race_key, hippo, cote, cote_ref, tendance, amplitude, favori, avis, is_win, place) = (
            row
        )
        ...
        if cote and cote > 0:
            ...
            if value <= 0:
                excluded.append(
                    {"nom": nom, "race_key": race_key, "reason": f"value ({value:.2%}) ≤ 0"}
                )
                continue
                ...
                excluded.append(
                    {
                        "nom": nom,
                        "race_key": race_key,
                        "reason": f"value ({value:.2%}) < cutoff ({value_cutoff:.0%})",
                    }
                )
                continue
                ...
                excluded.append(
                    {
                        "nom": nom,
                        "race_key": race_key,
                        "reason": f"max {max_bets_per_race} paris/course",
                    }
                )
                continue
            ...
            if stake > max_stake_per_bet:
                stake = (
                    math.floor(max_stake_per_bet / rounding) * rounding
                    if rounding > 0
                    else round(max_stake_per_bet, 2)
                )
            ...
            if total_stake + stake > daily_budget:
                ...
                if remaining >= min_stake_threshold:
                    ...
                    excluded.append(
                        {"nom": nom, "race_key": race_key, "reason": "budget jour épuisé"}
                    )
                    continue
        ...
        for pos in ordered_positions:
            ...
            if profil not in policy["allowed_profiles"]:
                excluded.append(
                    {
                        "nom": pos["nom"],
                        "race_key": pos["race_key"],
                        "reason": f"profil {profil} exclu pour bankroll {bankroll:.0f}€",
                    }
                )
                continue
                ...
                excluded.append(
                    {
                        "nom": pos["nom"],
                        "race_key": pos["race_key"],
                        "reason": "quota paris risqués atteint",
                    }
                )
                continue
                ...
                excluded.append(
                    {
                        "nom": pos["nom"],
                        "race_key": pos["race_key"],
                        "reason": f"max {policy['max_bets']} paris pour cette bankroll",
                    }
                )
                continue
                ...
                excluded.append(
                    {
                        "nom": pos["nom"],
                        "race_key": pos["race_key"],
                        "reason": f"p_win {pos.get('p_win', 0):.2f} < seuil {min_p_win:.2f}",
                    }
                )
                continue
            ...
            stake_pct = (
                stake_ref / ENGINE_BANKROLL_REFERENCE if ENGINE_BANKROLL_REFERENCE > 0 else 0
            )
            ...
            if total_stake + stake > daily_budget:
                    ...
                    excluded.append(
                        {
                            "nom": pos["nom"],
                            "race_key": pos["race_key"],
                            "reason": "budget journalier adapté épuisé",
                        }
                    )
                    continue
                if rounding > 0:
                ...
                if stake > cap_limit:
                    stake = (
                        cap_limit if rounding <= 0 else math.floor(cap_limit / rounding) * rounding
                    )
...
async def get_portfolio_today(
    ...
    try:
        ...
        if source_norm in ("picks", "picks_today", "today"):
            ...
            search_date = (
                picks_payload.get("date")
                if isinstance(picks_payload, dict)
                else datetime.now().strftime("%Y-%m-%d")
            )
            engine_meta = (
                (picks_payload.get("meta") or {}) if isinstance(picks_payload, dict) else {}
            )
            ...
            return {
                ...
                "caps": {
                    ...
                    "max_unit_bets_per_race": policy_result.get("policy", {}).get(
