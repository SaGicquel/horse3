...
class CalibrationState:
    ...
    alpha: float = 0.2
    alpha_by_disc: Dict[str, float] = field(default_factory=lambda: {
        'plat': 0.0,
        'trot': 0.4,
        'obstacle': 0.4,
        'global': 0.2
...
def check_yaml_artifacts_mismatch(
    ...
    if abs(yaml_temp - art_temp) > tolerance:
        result['mismatches'].append(f"temperature: YAML={yaml_temp:.4f} vs artifacts={art_temp:.4f}")
    ...
    if yaml_calibrator != art_calibrator:
        result['mismatches'].append(f"calibrator: YAML={yaml_calibrator} vs artifacts={art_calibrator}")
...
if __name__ == "__main__":
    ...
    if mismatch['has_mismatch']:
        ...
        print(f"   ✅ YAML et artefacts synchronisés")
