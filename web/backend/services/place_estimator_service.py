...
class PlaceEstimatorService:
        ...
        for i, p in enumerate(partants):
            enriched_partants.append({
                **p,
                ...
                'place_vs_win_ratio': round(float(p_place_3[i] / p_win_blend[i]), 2) if p_win_blend[i] > 0.001 else 0
            })
    ...
    def generate_packs(
        ...
        for i, ticket in enumerate(filtered_tickets[:config['n_tickets']]):
            ...
            numeros = [int(idx) + 1 for idx in combo_indices] if combo_indices else list(range(1, len(ticket['combo']) + 1))
        ...
        return {
            ...
            'analysis_meta': {
                ...
                'n_simulations': analysis.get('combo_stats', {}).get('n_simulations', self.num_simulations)
            }
        }
