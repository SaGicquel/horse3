...
class TicketOptimizer:
    ...
    def select_pack(
        ...
        if prefer_coverage:
            preferred = [c for c in candidates if prefer_coverage in c['classification']['coverage']]
        ...
        if pack_label == "RISQUÉ":
            ...
            candidates.sort(key=lambda x: x['stats']['ev'] * 0.7 + x['stats']['prob'] * 0.3, reverse=True)
        ...
        if budget > max_pack_budget:
            notes.append(f"Budget réduit de {budget:.2f}€ à {effective_budget:.2f}€ (max_pack_rate={self.config.max_pack_rate:.1%})")
        ...
        return {
            ...
            'portfolio_std': round(np.sqrt(portfolio_var), 2) if selected and portfolio_var > 0 else 0,
...
class ExoticTicketGenerator:
    ...
    def generate(
        ...
        combo_probs = {
            combo: count / num_simulations 
            for combo, count in combo_counts.items()
        }
        
        # Estimer les payouts et stats
        print(f"💰 Estimation des payouts...")
        if fixed_odds:
            ...
            combo_stats = self.analyzer.estimate_payouts(
                combo_probs, n_horses, structure, takeout
            )
        ...
        result = {
            ...
            'simulation_stats': {
                ...
                'coverage_rate': round(n_combos_total / self._theoretical_combos(n_horses, structure) * 100, 2)
            },
            'probabilities': {
                horse_names[i]: round(probs[i] * 100, 2) 
            ...
            'summary': {
                'total_stake': round(
                    ...
                    pack_risky['total_stake'], 2
                ),
                'total_tickets': (
                    pack_sure['n_tickets'] + 
                    pack_balanced['n_tickets'] + 
                    pack_risky['n_tickets']
                ),
                'avg_portfolio_ev_pct': round(np.mean([
...
def main():
    ...
    if args.test:
        ...
        horse_names = [
            "Tonnerre", "Éclair", "Tempête", "Ouragan", "Cyclone",
            "Rafale", "Bourrasque", "Zéphyr", "Brise", "Souffle"
        ]
        ...
        for pack in result['packs']:
            ...
            if pack['tickets']:
                print(f"      Exemple: {pack['tickets'][0]['combo']} (EV: {pack['tickets'][0]['ev_pct']})")
