...
def main():
    ...
    print(f"  • Chevaux en base: {etat_avant['nb_chevaux_avant']} → {etat_apres['nb_chevaux_apres']} (+{etat_apres['nb_chevaux_apres'] - etat_avant['nb_chevaux_avant']})")
    print(f"  • Courses en base: {etat_avant['nb_courses_avant']} → {etat_apres['nb_courses_apres']} (+{etat_apres['nb_courses_apres'] - etat_avant['nb_courses_avant']})")
    ...
    if total_ch > 0:
        ...
        print(f"    • {stats_ch[0]} avec stats trot ({stats_ch[0]*100//total_ch if total_ch > 0 else 0}%)")
        print(f"    • {stats_ch[1]} avec stats plat ({stats_ch[1]*100//total_ch if total_ch > 0 else 0}%)")
        print(f"    • {stats_ch[2]} avec gains annuels ({stats_ch[2]*100//total_ch if total_ch > 0 else 0}%)")
    print()
