...
def prepare_data():
    ...
    feature_cols = [col for col in train_df.columns if col not in target_cols + id_cols]
    ...
    if categorical_cols:
        ...
        for col in categorical_cols:
            ...
            all_values = pd.concat(
                [X_train_processed[col], X_val_processed[col], X_test_processed[col]]
            ).astype(str)
...
def train_comparison_models(data, target="victoire"):
    ...
    results["lightgbm"] = {"model": lgb_model, "auc_test": lgb_auc, "train_time": lgb_time}
    ...
    results["random_forest"] = {"model": rf_model, "auc_test": rf_auc, "train_time": rf_time}
...
def main():
    ...
    try:
        ...
        logging.info("   🔴 Ancien modèle AUC: ~0.932 (avec data leakage)")

        difference = (0.932 - auc_xgb) * 100
        logging.info(f"   📉 Baisse attendue: -{difference:.1f} points")
        logging.info("   ✅ Cette baisse confirme l'élimination du data leakage!")
