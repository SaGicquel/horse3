...
class OnlineTrainer:
    ...
    def __init__(
        ...
        self.feature_columns = [
            ...
            'nb_places_12m', 'derniere_place', 'derniere_victoire',
            # Aptitude (3)
    ...
    def load_feedback_data(self) -> pd.DataFrame:
        ...
        if len(feedback_df) < self.min_new_samples:
            logger.warning(f"⚠️  Seulement {len(feedback_df)} feedbacks (min: {self.min_new_samples})")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=7,
        help='Nombre de jours de feedback à inclure (défaut: 7)'
    )
    parser.add_argument(
        '--min-roc-auc',
        type=float,
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Test sans sauvegarder le modèle'
    )
