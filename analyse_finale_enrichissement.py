...
def analyser_colonnes_vides():
    ...
    taux_moyen = sum(t[3] for t in colonnes_pleines + colonnes_partielles + colonnes_vides) / len(
        colonnes
    )
    print(f"   Taux remplissage moyen : {taux_moyen:.1f}%")
    print(
        f"   Colonnes pleines       : {len(colonnes_pleines)}/{len(colonnes)} ({len(colonnes_pleines)*100//len(colonnes)}%)"
    )
    print(
        f"   Colonnes à enrichir    : {len(colonnes_partielles) + len(colonnes_vides)}/{len(colonnes)}"
    )
    ...
    if len(colonnes_vides) > 50:
        ...
        print("   → Vérifier si ces colonnes sont utiles ou peuvent être supprimées\n")
    ...
    if len(colonnes_partielles) > 0:
        ...
        for scraper, cols in scrapers_cols.items():
            cols_manquantes = [
                c for c in cols if any(c == pc[0] for pc in colonnes_partielles + colonnes_vides)
            ]
            if cols_manquantes:
...
if __name__ == "__main__":
    try:
        ...
        with open("ANALYSE_ENRICHISSEMENT_OCTOBRE_2024.txt", "w", encoding="utf-8") as f:
            f.write("ANALYSE ENRICHISSEMENT OCTOBRE 2024\n")
            ...
            f.write("COLONNES À ENRICHIR:\n")
            f.write("-" * 80 + "\n")
            for col_name, nb_null, nb_rempli, taux in resultat["colonnes_partielles"]:
                f.write(f"{taux:5.1f}% │ {col_name}\n")

        print("✅ Rapport sauvegardé dans: ANALYSE_ENRICHISSEMENT_OCTOBRE_2024.txt\n")
