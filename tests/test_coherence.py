...
def sample_config():
    ...
    return {
        ...
        "kelly": {"fraction": 0.25, "max_stake_pct": 0.05, "min_stake": 2.0, "value_cutoff": 0.05},
    }
...
def sample_artifact_report():
    ...
    return {
        ...
        "temperature": 1.254,
        "alpha": 0.2,
        "alpha_by_cluster": {"plat": 0.0, "trot": 0.4, "obstacle": 0.4},
        "best_calibrator": "platt",
        "metrics": {"brier_score": 0.0821, "log_loss": 0.312, "ece": 0.0082},
    }
...
class TestProbabilityNormalization:
    ...
    def test_harville_place_probabilities_constraint(self, sample_probabilities):
        ...
        for i in range(n):
            assert (
                place_probs[i] >= win_probs[i] - 1e-10
...
class TestConfigArtifactCoherence:
    ...
    def test_temperature_coherence(self, sample_config, sample_artifact_report):
        ...
        assert (
            abs(config_temp - artifact_temp) < 0.001
    ...
    def test_alpha_global_coherence(self, sample_config, sample_artifact_report):
        ...
        assert (
            abs(config_alpha - artifact_alpha) < 0.001
    ...
    def test_alpha_by_discipline_coherence(self, sample_config, sample_artifact_report):
        ...
        for disc in ["plat", "trot", "obstacle"]:
            if disc in config_alpha_disc and disc in artifact_alpha_disc:
                ...
                assert (
                    abs(config_val - artifact_val) < 0.001
...
class TestIntegration:
    ...
    def test_pro_betting_analyzer_uses_config(self, mock_db_connection):
        ...
        try:
            ...
            assert (
                abs(analyzer.temperature - expected_temp) < 0.001
    ...
    def test_race_pronostic_generator_uses_config(self, mock_db_connection):
        ...
        try:
            ...
            assert (
                abs(default_cfg["temperature"] - expected_temp) < 0.001
        ...
        try:
            ...
            for name1, t1 in temperatures.items():
                for name2, t2 in temperatures.items():
                    assert abs(t1 - t2) < 0.001, f"Incohérence T: {name1}={t1}, {name2}={t2}"
...
class TestNumSimulationsCoherence:
    ...
    def test_config_yaml_num_simulations(self):
        ...
        try:
            ...
            assert (
                config.simulation.num_simulations == self.EXPECTED_NUM_SIMULATIONS
    ...
    def test_race_pronostic_generator_num_simulations(self):
        ...
        try:
            ...
            assert (
                actual == self.EXPECTED_NUM_SIMULATIONS
    ...
    def test_exotic_ticket_generator_num_simulations(self):
        ...
        try:
            ...
            assert (
                config.num_simulations == self.EXPECTED_NUM_SIMULATIONS
    ...
    def test_monte_carlo_simulator_default(self):
        ...
        try:
            ...
            assert (
                sim.num_simulations == self.EXPECTED_NUM_SIMULATIONS
    ...
    def test_all_modules_same_num_simulations(self):
        ...
        assert len(unique_values) == 1, f"Incohérence num_simulations détectée: {values}"
    ...
    def test_fails_if_module_uses_different_n(self):
        ...
        checks = [
            (
                "config.yaml",
                lambda: __import__("config.loader").loader.get_config().simulation.num_simulations,
            ),
            (
                "race_pronostic_generator",
                lambda: __import__("race_pronostic_generator")
                .get_default_config()
                .get("num_simulations"),
            ),
            (
                "ExoticConfig",
                lambda: __import__("exotic_ticket_generator").ExoticConfig().num_simulations,
            ),
            (
                "MonteCarloSimulator",
                lambda: __import__("race_pronostic_generator")
        ...
        assert len(errors) == 0, "Modules avec num_simulations incorrect:\n" + "\n".join(errors)
