...
class TestFusionModel:
    ...
    def fusion_setup(self):
        ...
        data['jockey'].x = torch.randn(5, 8)
        data['entraineur'].x = torch.randn(3, 8)
        data['cheval', 'monte_par', 'jockey'].edge_index = torch.stack([torch.randint(0, 10, (20,)), torch.randint(0, 5, (20,))])
        data['jockey', 'monte', 'cheval'].edge_index = torch.stack([torch.randint(0, 5, (20,)), torch.randint(0, 10, (20,))])
        data['cheval', 'entraine_par', 'entraineur'].edge_index = torch.stack([torch.randint(0, 10, (20,)), torch.randint(0, 3, (20,))])
        data['entraineur', 'entraine', 'cheval'].edge_index = torch.stack([torch.randint(0, 3, (20,)), torch.randint(0, 10, (20,))])
        
        gnn = EntityGraphNN(
            metadata=data.metadata(), hidden_dim=16, out_dim=16, n_layers=2
        )
