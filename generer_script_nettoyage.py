...
COLONNES_CRITIQUES = {
    ...
    'cheval_courses_seen': ['id_cheval', 'race_key', 'date_course', 'position_arrivee', 'created_at']
}

# Colonnes utilisées par scrapers (détectées dans audit)
COLONNES_SCRAPERS = {
    'cheval_courses_seen': [
        'autostart_ligne', 'classe_course', 'code_hippodrome', 'course_id',
        'heure_locale', 'meeting_id', 'nom_hippodrome', 'pays_course',
        'profil_piste', 'reduction_km', 'rend_m', 'temps_total_s', 'type_course'
    ],
    'chevaux': []
}
...
COLONNES_CALCULEES = {
    'chevaux': [
        ...
        'nb_courses_total', 'nb_victoires', 'nb_places', 'taux_victoire', 'taux_place',
        'gains_total', 'gains_moyen', 'derniere_course', 'age_debut_carriere', 'indice_consistance_12m',
        # Jour 3 - Temporelles  
        'jours_depuis_derniere_course', 'forme_recente', 'regularite_courses',
        'progression_distance', 'performance_saison', 'tendance_last_5',
        # Jour 4 - Comparaisons
        'ecart_median_gagnant', 'meilleur_temps_relatif', 'rank_gains_generation',
        'performance_vs_moyenne_course', 'taux_victoire_vs_concurrent', 'handicap_optimal',
        # Jour 5 - ML Features
        'momentum_score', 'distance_preference_score', 'terrain_preference_score',
        'consistency_score', 'improvement_rate', 'versatility_score'
    ],
    'cheval_courses_seen': []
}
...
for table in ['chevaux', 'cheval_courses_seen']:
    ...
    if colonnes_a_supprimer:
        ...
        for i, (col, taux, raison) in enumerate(colonnes_a_supprimer):
            ...
            script_sql.append(f"  DROP COLUMN IF EXISTS {col} CASCADE{virgule}  -- {raison} ({taux:.1f}%)")
...
for table, stats in statistiques.items():
    ...
    print(f"  À supprimer:         {stats['a_supprimer']} ({stats['a_supprimer']/stats['total']*100:.1f}%)")
...
print(f"\n⚠️  AVANT D'EXÉCUTER:")
print(f"   1. Vérifier le script manuellement")
print(f"   2. Faire un backup: pg_dump -U postgres -d pmubdd > backup.sql")
print(f"   3. Tester sur copie base si possible")
