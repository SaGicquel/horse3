...
class MLFeatureEngineer:
    ...
    def calculate_forme_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for id_cheval in df['id_cheval'].unique():
            ...
            for idx in cheval_data.index:
                ...
                forme_10 = sum((6 - min(pos, 5)) * w for pos, w in zip(positions_10, weights_10[::-1]))
                ...
                if len(hist_12m) > 0:
                    ...
                    df.loc[idx, 'regularite'] = hist_12m['position_arrivee'].std() if len(hist_12m) > 1 else 0
        ...
        for id_cheval in df['id_cheval'].unique():
            ...
            for idx in cheval_data.index:
                ...
                if len(hist_distance) > 0:
                    df.loc[idx, 'aptitude_distance'] = (hist_distance['place'] == 1).sum() / len(hist_distance)
                
                # Aptitude piste - FIX: utiliser 'victoire' au lieu de 'place'
                if pd.notna(piste):
                    hist_piste = historique[historique['type_piste'] == piste]
                    if len(hist_piste) > 0:
                        df.loc[idx, 'aptitude_piste'] = (hist_piste['victoire'] == 1).sum() / len(hist_piste)
                
                # Aptitude hippodrome
                if pd.notna(hippodrome):
                    hist_hippodrome = historique[historique['id_hippodrome'] == hippodrome]
                    if len(hist_hippodrome) > 0:
                        df.loc[idx, 'aptitude_hippodrome'] = (hist_hippodrome['place'] == 1).sum() / len(hist_hippodrome)
    ...
    def calculate_jockey_entraineur_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for id_jockey in df['id_jockey'].dropna().unique():
            ...
            for idx in jockey_data.index:
                ...
                if len(historique) >= 5:  # Minimum 5 courses
                    df.loc[idx, 'taux_victoires_jockey'] = (historique['victoire'] == 1).sum() / len(historique)
                    df.loc[idx, 'taux_places_jockey'] = (historique['place'] == 1).sum() / len(historique)
        ...
        for id_entraineur in df['id_entraineur'].dropna().unique():
            ...
            for idx in entraineur_data.index:
                ...
                if len(historique) >= 5:
                    df.loc[idx, 'taux_victoires_entraineur'] = (historique['victoire'] == 1).sum() / len(historique)
                    df.loc[idx, 'taux_places_entraineur'] = (historique['place'] == 1).sum() / len(historique)
        ...
        for idx in df.index:
            ...
            if pd.notna(id_jockey):
                ...
                if len(duo_jockey) >= 3:
                    df.loc[idx, 'synergie_jockey_cheval'] = (duo_jockey['place'] == 1).sum() / len(duo_jockey)
            ...
            if pd.notna(id_entraineur):
                ...
                if len(duo_entraineur) >= 3:
                    df.loc[idx, 'synergie_entraineur_cheval'] = (duo_entraineur['place'] == 1).sum() / len(duo_entraineur)
    ...
    def add_market_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for id_course in df['id_course'].unique():
            ...
            if len(cotes_sp) > 0:
                df.loc[mask & df['cote_sp'].notna(), 'rang_cote_sp'] = df.loc[mask, 'cote_sp'].rank()
            
            # Rang cote Turf.bzh
            cotes_turf = df.loc[mask, 'cote_turfbzh'].dropna()
            if len(cotes_turf) > 0:
                df.loc[mask & df['cote_turfbzh'].notna(), 'rang_cote_turfbzh'] = df.loc[mask, 'cote_turfbzh'].rank()
        ...
        for id_cheval in df['id_cheval'].unique():
            ...
            for idx in cheval_data.index:
                ...
                gains = historique.apply(
                    lambda x: (x['allocation'] or 0) * (0.5 if x['position_arrivee'] == 1 
                              else 0.25 if x['position_arrivee'] == 2 
                              else 0.15 if x['position_arrivee'] == 3 else 0), axis=1
                ).sum()
                ...
                nb_places = ((historique['position_arrivee'] >= 1) & (historique['position_arrivee'] <= 3)).sum()
                df.loc[idx, 'taux_places_carriere'] = nb_places / nb_courses if nb_courses > 0 else 0
                ...
                hist_ancien = historique[(historique['date_course'] < date_6m) & 
                ...
                if len(hist_recent) > 0 and len(hist_ancien) > 0:
                    gains_recent = hist_recent.apply(
                        lambda x: (x['allocation'] or 0) * (0.5 if x['position_arrivee'] == 1 else 0.25 if x['position_arrivee'] == 2 else 0.15 if x['position_arrivee'] == 3 else 0), axis=1
                    ).sum()
        ...
        for id_cheval in df['id_cheval'].unique():
            ...
            for idx in cheval_data.index:
                ...
                if len(historique) > 0 and pd.notna(etat):
                    ...
                    if len(hist_etat) >= 3:
                        df.loc[idx, 'aptitude_piste_etat'] = (hist_etat['victoire'] == 1).sum() / len(hist_etat)
    ...
    def add_onehot_encoding(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        hippodrome_dummies = pd.get_dummies(df['hippodrome_top20'], prefix='hippodrome', drop_first=True)
        df = pd.concat([df, hippodrome_dummies], axis=1)
        
        logger.info(f"   ✅ {len(discipline_dummies.columns) + len(sexe_dummies.columns) + len(piste_dummies.columns) + len(etat_dummies.columns) + len(hippodrome_dummies.columns)} colonnes one-hot créées")
        return df
    ...
    def calculate_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df.loc[:, 'interaction_aptitude_distance'] = df['aptitude_distance'] * df['distance_norm'].abs()
        ...
        df.loc[:, 'interaction_victoires_jockey'] = df['nb_victoires_12m'] * df['taux_victoires_jockey']
        ...
        df.loc[:, 'interaction_aptitude_popularite'] = df['aptitude_hippodrome'] * df['popularite_hippodrome']
...
def main():
    ...
    parser.add_argument('--output', default='data/ml_features.csv', 
    ...
    try:
        ...
        feature_groups = {
            'Forme': ['forme_5c', 'forme_10c', 'nb_courses_12m', 'nb_victoires_12m', 
                     'taux_places_12m', 'regularite', 'jours_depuis_derniere'],
            'Aptitude': ['aptitude_distance', 'aptitude_piste', 'aptitude_hippodrome'],
            'Jockey/Entraineur': ['taux_victoires_jockey', 'taux_places_jockey', 
            ...
            'Marché': ['rang_cote_sp', 'rang_cote_turfbzh', 'ecart_cote_ia', 
                      'elo_cheval', 'prediction_ia_gagnant']
        }
