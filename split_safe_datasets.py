...
def split_dataset_safe():
    ...
    try:
        ...
        assert len(df_train) + len(df_val) + len(df_test) == len(
            df
        ), "Perte de données dans le split!"
        ...
        for dataset_name, dataset in [("TRAIN", df_train), ("VAL", df_val), ("TEST", df_test)]:
            ...
            logging.info(
                f"      🏆 Victoires: {dataset['victoire'].sum():,} ({victoires_pct:.1f}%)"
            )
        ...
        logging.info("   ✅ Train sauvé: data/train_SAFE.csv")

        df_val.to_csv("data/val_SAFE.csv", index=False)
        logging.info("   ✅ Validation sauvé: data/val_SAFE.csv")

        df_test.to_csv("data/test_SAFE.csv", index=False)
        logging.info("   ✅ Test sauvé: data/test_SAFE.csv")

        # Résumé final
        logging.info("\n📋 RÉSUMÉ FINAL:")
        logging.info("   📂 Fichiers générés: 3 datasets")
        logging.info(f"   📊 Colonnes par dataset: {df_train.shape[1]}")
        logging.info("   🎯 Targets disponibles: position_arrivee, victoire, place")
        logging.info("   📅 Split temporel: Garantie anti-leakage")
