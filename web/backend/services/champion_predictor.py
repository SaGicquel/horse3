...
class ChampionArtifacts:
    ...
    platt_calibrator_path: Optional[str] = None
...
class ChampionPredictor:
    ...
    def load(self) -> None:
        ...
        for path in (self.artifacts.model_path, self.artifacts.scaler_path, self.artifacts.imputer_path):
        ...
        self._expected_n_features = getattr(self._model, "n_features_in_", None) or getattr(self._scaler, "n_features_in_", None)

        # Certains artefacts (imputer) peuvent être incompatibles ou entraînés sur un sous-ensemble.
        if self._expected_n_features and getattr(self._imputer, "n_features_in_", None) not in (None, self._expected_n_features):
            self._imputer = None
        ...
        if not self._feature_names and self.artifacts.feature_names_path and os.path.exists(self.artifacts.feature_names_path):
            try:
                with open(self.artifacts.feature_names_path, "r", encoding="utf-8") as f:
                    names = json.load(f)
        ...
        if metadata_path and os.path.exists(metadata_path):
            try:
                ...
                if report_rel and not self.artifacts.calibration_report_path:
                    self.artifacts.calibration_report_path = os.path.normpath(os.path.join(base_dir, report_rel))
                if platt_rel and not self.artifacts.platt_calibrator_path:
                    self.artifacts.platt_calibrator_path = os.path.normpath(os.path.join(base_dir, platt_rel))
            except Exception:
                pass
    ...
    def predict_proba(self, rows: List[Dict[str, Any]]) -> List[float]:
        ...
        if self._scaler is not None:
            try:
                ...
                # Fallback manuel (évite les incompatibilités sklearn pickles)
                try:
        ...
        # 1) Récupérer les "logits" si possible (output_margin), sinon logit(proba)
        margins = None
        try:
        ...
        # 2) Temperature softmax par course (ici: rows = 1 course, donc une normalisation)
        ...
        # 3) Calibration Platt (optionnelle), puis renormalisation
        p_cal = p_norm
...
def default_champion_artifacts() -> ChampionArtifacts:
    ...
    return ChampionArtifacts(
        ...
        calibration_report_path=os.getenv("CHAMPION_CALIBRATION_REPORT", "/project/calibration/champion/calibration_report.json"),
        platt_calibrator_path=os.getenv("CHAMPION_PLATT_PATH", "/project/calibration/champion/calibrator_platt.pkl"),
