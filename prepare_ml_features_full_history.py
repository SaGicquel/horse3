...
class FullHistoryMLFeatureEngineer:
    ...
    def calculate_cheval_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i, (cheval, cheval_data) in enumerate(df.groupby('nom_norm')):
            ...
            for idx in cheval_data.index:
                ...
                if len(recent_5) > 0:
                    df.loc[df.index[df['nom_norm'] == cheval][course_idx], 'forme_5c'] = \
                        recent_5['victoire'].mean()
                
                # Forme sur les 10 dernières courses  
                recent_10 = historique.tail(10)
                if len(recent_10) > 0:
                    df.loc[df.index[df['nom_norm'] == cheval][course_idx], 'forme_10c'] = \
                        recent_10['victoire'].mean()
                ...
                if len(hist_12m) > 0:
                    ...
                    df.loc[df.index[df['nom_norm'] == cheval][course_idx], 'nb_courses_12m'] = nb_courses
                    df.loc[df.index[df['nom_norm'] == cheval][course_idx], 'nb_victoires_12m'] = nb_victoires
                ...
                if len(historique) >= 3:
                    df.loc[df.index[df['nom_norm'] == cheval][course_idx], 'regularite'] = \
                        1.0 / (1.0 + historique['position_arrivee'].std())
                
                # Jours depuis dernière course
                if len(historique) > 0:
                    derniere_course = historique['date_course'].max()
                    jours = (date_course - derniere_course).days
                    df.loc[df.index[df['nom_norm'] == cheval][course_idx], 'jours_depuis_derniere'] = jours
    ...
    def calculate_jockey_entraineur_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.merge(jockey_stats[['driver_jockey', 'taux_victoires', 'taux_places']], 
                     on='driver_jockey', how='left', suffixes=('', '_jockey'))
        ...
        entraineur_stats['taux_victoires'] = entraineur_stats['nb_victoires'] / entraineur_stats['nb_courses']
        entraineur_stats['taux_places'] = entraineur_stats['nb_places'] / entraineur_stats['nb_courses']
        
        # Merge avec le DataFrame principal
        df = df.merge(entraineur_stats[['entraineur', 'taux_victoires', 'taux_places']], 
                     on='entraineur', how='left', suffixes=('', '_entraineur'))
        ...
        df['synergie_entraineur_cheval'] = (df['taux_victoires_entraineur'] * df['forme_5c']).fillna(0)
    ...
    def calculate_course_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['niveau_moyen_concurrent'] = df['niveau_moyen_concurrent'].fillna(df['niveau_moyen_concurrent'].mean())
    ...
    def add_interaction_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        hippo_popularity = df.groupby('hippodrome_nom').size().reset_index(name='popularite_hippodrome')
...
def main():
    ...
    parser.add_argument('--output', default='data/ml_features_full_history.csv', 
