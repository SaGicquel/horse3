...
class AuditeurColonnes:
        ...
        try:
            with self.conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                return {
                    ...
                    'taux_remplissage': float(result['taux_remplissage']) if result['taux_remplissage'] else 0.0
                }
        except Exception as e:
    ...
    def scanner_scrapers(self):
        ...
        patterns = {
            'UPDATE': re.compile(r'UPDATE\s+(?:chevaux|cheval_courses_seen)\s+SET\s+([^=]+)\s*=', re.IGNORECASE),
            'INSERT_COLS': re.compile(r'INSERT\s+INTO\s+(?:chevaux|cheval_courses_seen)\s*\(([^)]+)\)', re.IGNORECASE),
            'DICT_KEY': re.compile(r'["\']([a-z_]+)["\']\s*[:=]'),  # Clés de dictionnaire
            'SQL_COL': re.compile(r'\b([a-z_]+)\s*=\s*%s', re.IGNORECASE),  # Colonnes SQL avec placeholder
        }
            ...
            try:
                with open(filepath, 'r', encoding='utf-8') as f:
                    ...
                    for col in colonnes_trouvees:
                        if re.match(r'^[a-z][a-z0-9_]{2,}$', col) and col not in ['select', 'from', 'where', 'and', 'or', 'values']:
    ...
    def detecter_doublons_semantiques(self):
        ...
        groupes = {
            ...
            'forme_recente': ['forme_recente', 'forme_recente_30j', 'forme_recente_60j', 'forme_recente_90j', 'score_forme_recent'],
            'derniere_course': ['derniere_course', 'derniere_course_date', 'date_derniere_course'],
            'gains': ['gains_total', 'gains_carriere', 'gains_trot', 'gains_plat', 'gains_obstacle'],
            'elo': ['elo_courant', 'elo_avant_course'],
            'cotes': ['cote_matin', 'cote_finale', 'cote_finale_dec', 'cote_ouverture_dec', 'cote_live_dec'],
            'temps': ['temps_str', 'temps_sec', 'temps_total_s'],
            'reduction_km': ['reduction_km_sec', 'reduction_km', 'reduction_km_record_trot', 'reduction_km_record_plat'],
        ...
        for groupe_nom, colonnes in groupes.items():
            ...
            # Recommandation
            colonnes_existantes = [c for c in colonnes 
    ...
    def generer_rapport_complet(self):
        ...
        for table, colonnes in [('chevaux', self.colonnes_chevaux), 
            ...
            for col_info in colonnes:
                ...
                print(f"\n{col_name:35s} {statut}")
                print(f"   Taux: {taux:6.2f}% | {stats['non_null']:>8,} / {stats['total']:>8,} | Distinct: {stats['distinct']:>8,}")
    ...
    def generer_rapport_markdown(self, doublons):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            vides_chevaux = sum(1 for col in self.colonnes_chevaux 
                               if self.stats_colonnes.get(f"chevaux.{col['column_name']}", {}).get('taux_remplissage', 100) == 0)
            vides_courses = sum(1 for col in self.colonnes_courses 
                               if self.stats_colonnes.get(f"cheval_courses_seen.{col['column_name']}", {}).get('taux_remplissage', 100) == 0)
            
            # Compter colonnes critiques (<1%)
            critiques_chevaux = sum(1 for col in self.colonnes_chevaux 
                                   if 0 < self.stats_colonnes.get(f"chevaux.{col['column_name']}", {}).get('taux_remplissage', 100) < 1)
            critiques_courses = sum(1 for col in self.colonnes_courses 
                                   if 0 < self.stats_colonnes.get(f"cheval_courses_seen.{col['column_name']}", {}).get('taux_remplissage', 100) < 1)
            
            f.write(f"| Métrique | Valeur |\n")
            f.write(f"|----------|--------|\n")
            f.write(f"| **Total colonnes** | {total_cols} ({total_cols_chevaux} chevaux + {total_cols_courses} courses) |\n")
            f.write(f"| **Colonnes 100% vides** | {vides_chevaux + vides_courses} ({vides_chevaux} chevaux + {vides_courses} courses) |\n")
            f.write(f"| **Colonnes critiques (<1%)** | {critiques_chevaux + critiques_courses} ({critiques_chevaux} chevaux + {critiques_courses} courses) |\n")
            ...
            for table, colonnes in [('chevaux', self.colonnes_chevaux), 
                ...
                for col_name, col_info, stats in colonnes_triees:
                    ...
                    f.write(f"| {emoji} `{col_name}` | {taux:.1f}% | {non_null:,} | {total:,} | {distinct:,} | {dtype} | {source} |\n")
                    ...
                    for col in colonnes:
                        for table in ['chevaux', 'cheval_courses_seen']:
                            ...
                            if key in self.stats_colonnes:
                                ...
                                f.write(f"- `{col}` ({table}): {stats['taux_remplissage']:.1f}% rempli\n")
            else:
            ...
            for table, colonnes in [('chevaux', self.colonnes_chevaux), 
                                   ('cheval_courses_seen', self.colonnes_courses)]:
                cols_vides = [col['column_name'] for col in colonnes 
                             if self.stats_colonnes.get(f"{table}.{col['column_name']}", {}).get('taux_remplissage', 100) == 0]
                if cols_vides:
                    f.write(f"\n**Table `{table}`**:\n")
            ...
            for table, colonnes in [('chevaux', self.colonnes_chevaux), 
                                   ('cheval_courses_seen', self.colonnes_courses)]:
                cols_critiques = [(col['column_name'], self.stats_colonnes.get(f"{table}.{col['column_name']}", {})) 
                                 for col in colonnes 
                                 if 0 < self.stats_colonnes.get(f"{table}.{col['column_name']}", {}).get('taux_remplissage', 100) < 1]
                if cols_critiques:
                    f.write(f"\n**Table `{table}`**:\n")
