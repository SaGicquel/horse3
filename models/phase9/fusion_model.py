...
class FusionTransformerGNN(nn.Module):
    ...
    def __init__(
        self, transformer_model, gnn_model, transformer_dim, gnn_dim, fusion_dim=64, dropout=0.1
    ):
        """
        Args:
    ...
    def forward(
        self, temporal_src, temporal_mask, graph_x_dict, graph_edge_index_dict, batch_indices
    ):
        """
        Args:
        ...
        embedded_src = self.transformer.input_embedding(temporal_src) * torch.sqrt(
            torch.tensor(self.transformer.d_model)
        )
        pos_encoded_src = self.transformer.pos_encoder(embedded_src.transpose(0, 1)).transpose(0, 1)
        transformer_output = self.transformer.transformer_encoder(
            pos_encoded_src, src_key_padding_mask=temporal_mask
        )

        # Pooling (Moyenne masquée)
        mask_expanded = ~temporal_mask.unsqueeze(-1)
        temporal_emb_raw = (transformer_output * mask_expanded).sum(dim=1) / mask_expanded.sum(
            dim=1
        )
        ...
        for node_type, x in graph_x_dict.items():
            ...
            x_dict_out[node_type] = torch.dropout(
                x_dict_out[node_type], p=self.gnn.dropout, train=self.gnn.training
            )
