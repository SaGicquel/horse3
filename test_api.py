...
def test_prediction(base_url: str, verbose: bool = False) -> bool:
    ...
    try:
        ...
        for pred in data['top_3']:
            print(f"   {pred['rang_prediction']:<6} "
        ...
        assert len(data['toutes_predictions']) == len(course['partants']), "Toutes prédictions incohérentes"
        assert all(0 <= p['probabilite_victoire'] <= 1 for p in data['toutes_predictions']), "Probabilités invalides"
...
def main():
    ...
    parser.add_argument('--load-test', action='store_true',
                       help='Effectuer un test de charge (100 requêtes)')
