                                ...
                                for k in range(n):
                                    if k != i and k != j:
                                        ...
                                        if remaining_2 > 1e-10:
                                            ...
                                            prob_i += (
                                                win_probs[j] * p_k_given_j * p_i_given_jk * 0.1
                                            )
...
class BayesianOddsFusion:
    ...
    def fuse_probabilities(
        self, market_probs: np.ndarray, model_probs: np.ndarray, confidence: float = 1.0
    ) -> np.ndarray:
        """
...
class ProBettingAnalyzer:
    ...
    def __init__(
        ...
        if CONFIG_AVAILABLE:
            ...
            logger.info(
                f"✅ ProBettingAnalyzer init: T={softmax_temperature:.4f}, "
    ...
    def _get_runner_raw_score(
        self, nom: str, cote: float, distance: int, hippodrome: str, data: dict
        ...
        if row and row[0] and row[0] >= 2 and row[1] and row[1] >= 1:
            score += 10
            rationale.append("🏟️ Déjà gagné ici")
    ...
    def analyze_race(self, race_key: str) -> str:
        ...
        for row in runners_data:
            ...
            score, rationale = self._get_runner_raw_score(nom, cote, distance, hippodrome, data)

            raw_scores.append(score)
            market_odds.append(cote if cote and cote > 1 else 10.0)
            runner_info.append(
                {"nom": nom, "numero": numero or 0, "cote": cote, "rationale": rationale}
            )
        ...
        # 7. Fusion bayésienne
        fused_probs = self.fusion.fuse_probabilities(market_probs, model_probs, confidence=0.8)
