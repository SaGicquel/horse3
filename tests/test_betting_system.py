...
class TestProbSumPerRace:
    ...
    def test_prob_sum_after_blend(self, sample_race_data):
        ...
        assert np.isclose(
            p_blend_norm.sum(), 1.0, atol=1e-6
...
class TestTemperatureFromArtifacts:
    ...
    def test_temperature_from_health_json(self, health_json_path):
        ...
        try:
            ...
            assert np.isclose(
                state.temperature, expected_temp, atol=1e-6
            ), f"T chargé = {state.temperature}, attendu = {expected_temp}"
            assert (
                state.source == "artifacts"
    ...
    def test_temperature_overrides_yaml(self, health_json_path):
        ...
        try:
            ...
            assert np.isclose(
                state_artifacts.temperature, health["temperature"], atol=1e-6
...
class TestAlphaFromArtifacts:
    ...
    def test_alpha_from_health_json(self, health_json_path):
        ...
        try:
            ...
            assert np.isclose(
                state.alpha, expected_alpha, atol=1e-6
    ...
    def test_alpha_by_discipline_from_artifacts(self, health_json_path):
        ...
        try:
            ...
            for key in expected_keys:
                assert key in state.alpha_by_disc, f"Clé '{key}' manquante dans alpha_by_disc"

            # Vérifier get_alpha_for_discipline
            for disc in ["plat", "trot", "obstacle"]:
                alpha = state.get_alpha_for_discipline(disc)
                assert 0.0 <= alpha <= 1.0, f"α pour {disc} hors bornes: {alpha}"
        except ImportError:
            pytest.skip("artifacts_loader non disponible")
...
class TestCoherenceGuard:
    ...
    def test_mismatch_detection(self, health_json_path):
        ...
        try:
            ...
            if result["has_mismatch"]:
                assert len(result["mismatches"]) > 0, "has_mismatch=True mais mismatches vide"
    ...
    def test_override_applied_when_mismatch(self, health_json_path):
        ...
        try:
            ...
            assert np.isclose(
                state.temperature, health["temperature"], atol=1e-6
            ), "T devrait être l'override des artefacts"
            assert np.isclose(
                state.alpha, health["alpha"], atol=1e-6
...
class TestTemperatureFromConfig:
    ...
    def test_temperature_from_config_yaml(self):
        ...
        try:
            ...
            assert (
                temp == self.EXPECTED_TEMPERATURE
    ...
    def test_temperature_bounds(self):
        ...
        assert (
            0.5 <= self.EXPECTED_TEMPERATURE <= 5.0
...
class TestAlphaByDiscipline:
    ...
    def test_alpha_plat(self):
        ...
        try:
            ...
            assert (
                alpha_plat == self.EXPECTED_ALPHA["plat"]
    ...
    def test_alpha_trot(self):
        ...
        try:
            ...
            alpha_by_disc = {}
            if hasattr(config, "calibration") and hasattr(
                config.calibration, "blend_alpha_by_discipline"
            ):
            ...
            assert (
                alpha_trot == self.EXPECTED_ALPHA["trot"]
    ...
    def test_alpha_obstacle(self):
        ...
        try:
            ...
            alpha_obs = alpha_by_disc.get(
                "obstacle", alpha_by_disc.get("haie", alpha_by_disc.get("steeplechase", 0.4))
            )

            assert (
                alpha_obs == self.EXPECTED_ALPHA["obstacle"]
    ...
    def test_alpha_blend_effect(self, sample_race_data):
        ...
        assert np.allclose(
            p_alpha_0 / p_alpha_0.sum(), p_market, atol=0.01
        ), "α=0 devrait être proche du marché"

        # Vérifier que α=1 est proche du modèle
        assert np.allclose(
            p_alpha_1 / p_alpha_1.sum(), p_model, atol=0.01
        ...
        assert (
            diff_to_market > 0.001 and diff_to_model > 0.001
...
class TestMCRunsConsistent:
    ...
    def test_mc_n_in_config(self):
        ...
        try:
            ...
            assert n == self.EXPECTED_N, f"N simulations = {n}, attendu {self.EXPECTED_N}"
    ...
    def test_mc_n_in_exotic_generator(self):
        ...
        assert (
            config.num_simulations == self.EXPECTED_N
    ...
    def test_mc_simulation_runs(self):
        ...
        assert (
            results.shape[0] == self.EXPECTED_N
    ...
    def test_mc_convergence(self):
        ...
        assert (
            np.abs(empirical_prob - probs[0]) < 0.01
...
class TestParimutuelEV:
    ...
    def test_fair_odds_pool_formula(self):
        ...
        assert np.isclose(
            fair_odds, expected_fair_odds, atol=1e-6
    ...
    def test_ev_with_takeout(self):
        ...
        assert np.isclose(
            ev_at_fair, -self.TAKEOUT, atol=1e-6
    ...
    def test_takeout_in_exotic_generator(self):
        ...
        assert (
            config.takeout_rate == self.TAKEOUT
...
class TestNoLeakage:
    ...
    def test_no_post_off_in_feature_list(self):
        ...
        for feat in inference_features:
            assert (
                feat not in self.POST_OFF_FEATURES
    ...
    def test_inference_vs_training_features(self):
        ...
        assert (
            training_target not in training_features
...
class TestKellyCapAndValue:
    ...
    def test_kelly_formula_correctness(self):
        ...
        assert np.isclose(
            kelly, expected_kelly, atol=1e-6
    ...
    def test_bet_simulator_respects_cap(self):
        ...
        assert (
            expected_stake_pct == self.MAX_STAKE_PCT
        ), "Pour ce pari très profitable, le cap devrait s'appliquer"
