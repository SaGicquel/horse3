...
print(f'Stats p_final: min={df_calibrated["p_final"].min():.3f}, max={df_calibrated["p_final"].max():.3f}, mean={df_calibrated["p_final"].mean():.3f}')

# Vérifier que les probabilités somment à 1 par course
sums_by_race = df_calibrated.groupby('race_id')['p_final'].sum()
print(f'Vérification normalisation: min={sums_by_race.min():.3f}, max={sums_by_race.max():.3f}, mean={sums_by_race.mean():.3f}')
