...
def derive_etat_piste(
    penetrometre: Optional[float], meteo: Optional[str], hippodrome: Optional[str]
) -> Tuple[str, str]:
    """
...
def proxy_temps_sec(
    distance_m: Optional[int], discipline: Optional[str], hippodrome: Optional[str]
) -> Optional[float]:
    """
...
def scrape_rapport_gagnant(
    race_id: str, horse_number: int, max_retries: int = 2, timeout: float = 5.0
...
class FillMissingFieldsJob:
    ...
    def __init__(self, conn=None, batch_size: int = 1000, dry_run: bool = False):
...
def main():
    ...
    parser.add_argument(
        "--field",
        choices=["etat_piste", "temps_sec", "rapport_gagnant", "gains_carriere", "all"],
        default="all",
        help="Champ à remplir",
    )

    args = parser.parse_args()

    job = FillMissingFieldsJob(batch_size=args.batch_size, dry_run=args.dry_run)
