...
def test_scraping_enrichi():
    ...
    for row in rows:
        ...
        if cote:
            stats["cote_finale"] += 1
        if cote_ref:
            stats["cote_reference"] += 1
        if tendance:
            stats["tendance_cote"] += 1
        if amplitude:
            stats["amplitude_tendance"] += 1
        if favori:
            stats["est_favori"] += 1
        if avis:
            stats["avis_entraineur"] += 1
        if allure:
            stats["allure"] += 1
        if url:
            stats["url_casaque"] += 1
        if duree:
            stats["duree_course"] += 1

        fav_str = "⭐" if favori else ""
        print(
            f"  {nom:<20} {cote or '-':<8} {cote_ref or '-':<8} {tendance or '-':<6} {fav_str:<5} {avis or '-':<10}"
        )
