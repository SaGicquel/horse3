...
def scrape_today(multi_thread=True):
    ...
    verify = input("\n💡 Voulez-vous vérifier l'intégrité de la base maintenant ? (O/n): ").strip().lower()
    if verify != 'n':
        verify_database()
    else:
...
def show_stats():
    ...
    import sqlite3
    from scraper_pmu_simple import DB_PATH
    ...
    # Stats 2025
    cur.execute("SELECT SUM(nombre_courses_2025), SUM(nombre_victoires_2025) FROM chevaux")
