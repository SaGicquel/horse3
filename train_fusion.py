...
class FusionDataset(Dataset):
    def __init__(self, temporal_data, graph_mappings):
        ...
        for idx in range(len(self.sequences)):
            ...
            if (
                h_id in self.mappings["cheval_map"]
                and j_id in self.mappings["jockey_map"]
                and e_id in self.mappings["entraineur_map"]
            ):
        ...
        print(
            f"Données alignées : {len(self.valid_indices)} / {len(self.sequences)} échantillons conservés."
        )
...
def main():
    ...
    print(
        f"Modèle Fusion initialisé. Params: {sum(p.numel() for p in fusion_model.parameters() if p.requires_grad):,}"
    )
    ...
    for epoch in range(EPOCHS):
        ...
        train_loss = train_one_epoch(
            fusion_model, train_loader, optimizer, criterion, graph, device
        )
