...
def main():
    ...
    parser = argparse.ArgumentParser(description="Enrichissement batch Turf.bzh pour une période")
    ...
    group.add_argument("--days", type=int, help="Nombre de jours en arrière depuis aujourd'hui")
    ...
    parser.add_argument(
        "--max-courses", type=int, help="Nombre maximum de courses à traiter (pour tests)"
    )
    ...
    logger.info("=" * 90)
    logger.info("🚀 ENRICHISSEMENT BATCH TURF.BZH")
    ...
    for i, (date, reunion, course) in enumerate(courses, 1):
        ...
        try:
            ...
            if enriched > 0:
                ...
                logger.info("   ⏭️  Aucune donnée Turf.bzh disponible")
            ...
            logger.info("   ⏭️  Passage à la course suivante")
    ...
    logger.info(
        f"   Courses enrichies       : {courses_enrichies} ({courses_enrichies/total_courses*100:.1f}%)"
    )
    logger.info(
        f"   Performances enrichies  : {perfs_enrichies_total}/{perfs_total} ({perfs_enrichies_total/perfs_total*100:.1f}% si > 0 else 0)"
    )
    logger.info("=" * 90)
