#!/usr/bin/env python3
"""
train_xgboost.py - Entraînement XGBoost pour prédiction victoires PMU

Phase 5 : Modèles Avancés
XGBoost améliore généralement RandomForest sur données tabulaires grâce au gradient boosting.

Usage:
    python train_xgboost.py --input-dir data/normalized --output-dir data/models
...
def load_splits(input_dir: Path) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if not all(p.exists() for p in [train_path, val_path, test_path]):
        raise FileNotFoundError(
            f"❌ Fichiers manquants dans {input_dir}. "
            "Exécutez normalize_features.py d'abord."
        )
...
def prepare_features(df: pd.DataFrame, exclude_patterns: list = None, feature_names: list = None) -> Tuple[np.ndarray, np.ndarray, list]:
    """
...
def train_xgboost(
    ...
    """
    Entraîne un modèle XGBoost avec early stopping.
    ...
    default_params = {
        ...
        'min_child_weight': 3,
        'gamma': 0.1,
        'scale_pos_weight': (len(y_train) - y_train.sum()) / y_train.sum(),  # Gère déséquilibre
        'random_state': 42,
        'n_jobs': -1,
        'eval_metric': 'logloss',
...
def plot_roc_curve(
    ...
    plt.plot(fpr, tpr, linewidth=2, label=f'XGBoost (AUC = {auc:.4f})', color='#2E86AB')
    ...
    plt.title('Courbe ROC - XGBoost', fontsize=13, fontweight='bold')
    plt.legend(loc='lower right', fontsize=10)
    plt.grid(alpha=0.3)
    plt.tight_layout()
    
    output_path = output_dir / "xgboost_roc_curve.png"
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def save_model_and_metrics(
    ...
    logger.info("")
    logger.info("🎉 Entraînement XGBoost terminé avec succès !")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Entraînement XGBoost pour prédiction victoires PMU"
    )
    parser.add_argument(
        '--input-dir',
        type=str,
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info("🚀 XGBOOST - PHASE 5 MODÈLES AVANCÉS")
    logger.info("=" * 80)
    ...
    try:
        ...
        save_model_and_metrics(
            model, metrics_train, metrics_val, metrics_test,
            feature_names, output_dir
        )
        
        logger.info("")
        logger.info("=" * 80)
        logger.info("✅ RÉSUMÉ FINAL - XGBOOST")
        logger.info("=" * 80)
