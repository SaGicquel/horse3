...
from enrichment.calculations import compute_annual_gains, compute_total_gains, compute_records
...
def cmd_init_db(args):
    ...
    try:
        ...
        print(
            "   💡 Utilisez 'python scraper_pmu_simple.py' pour créer/mettre à jour le schéma automatiquement."
        )
    ...
    try:
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                """,
                    (name, name_norm, sex, birth_date or None, birth_year, country, breed, coat),
                )
...
def cmd_recompute(args):
    ...
    cur.execute(
        "SELECT to_regclass('public.performances')"
    )  # Ou cheval_courses_seen selon le schéma
...
def cmd_calibrate(args):
    ...
    try:
        ...
        config = CalibrationConfig(
            artifacts_dir=str(calibration_dir), temperature_init=1.5, min_samples_cluster=100
        )
        ...
        if len(rows) < 500:
            print(
                f"⚠️  Pas assez de données ({len(rows)} lignes). Utilisation de données synthétiques."
            )
        ...
        if sync_config:
            ...
            try:
                ...
                print("   ⚠️  Module artifacts_loader non disponible")
            except Exception as e:
...
def cmd_health(args):
    ...
    try:
        ...
        try:
            ...
            if mismatch["has_mismatch"]:
                ...
                for diff in mismatch["mismatches"]:
                    print(f"      - {diff}")
                print("   💡 Exécutez: python cli.py calibrate --sync-config")
            else:
        ...
        print(
            f"   • ECE:         {metrics.get('ece', 'N/A'):.6f}"
            if isinstance(metrics.get("ece"), (int, float))
            else "   • ECE: N/A"
        )
        print(
            f"   • Log Loss:    {metrics.get('log_loss', 'N/A'):.6f}"
            if isinstance(metrics.get("log_loss"), (int, float))
            else "   • Log Loss: N/A"
        )
...
def cmd_pick(args):
    ...
    try:
        ...
        if resp.ok:
            ...
            print(
                f"⚠️ Backend /picks/today indisponible ({resp.status_code}), fallback heuristique."
            )
    except Exception as e:
    ...
    try:
        ...
        for race_key, course_nom, hippodrome, discipline in races:
            ...
            top3 = [
                {"horse": horses[i], "prob": round(float(p_blend[i]) * 100, 1)}
            ...
            for i, (p, o, h) in enumerate(zip(p_blend, odds, horses)):
                ...
                if ev > 0.05:  # > 5%
                    ...
                    stake = (
                        min(kelly * config["kelly_fraction"], config["max_stake_pct"]) * 100
                    )  # En €
...
def cmd_exotic(args):
    ...
    try:
        ...
        config = ExoticConfig(
            num_simulations=20000, budget=100.0, max_tickets=30, structure="trio_ordre"
        )

        # Générer les tickets
        generator = ExoticTicketGenerator(config)
        result = generator.generate(
            probabilities=probabilities.tolist(), horse_names=horses, structure="trio_ordre"
        )
        ...
        if paper_log_path.exists():
            ...
            if "odds" in df.columns and "pnl" in df.columns:
                ...
                for decile in sorted(df["odds_decile"].unique()):
                    ...
                    ev = (
                        subset["pnl"].sum() / subset["stake"].sum()
                        if "stake" in subset.columns
                        else 0
                    )
                    n = len(subset)
                    print(
                        f"      D{decile+1} (cotes ~{avg_odds:.1f}): EV {ev*100:+.1f}% ({n} paris)"
                    )
        else:
...
def handle_scraping_menu():
    ...
    while True:
        ...
        if choice == "0":
            ...
            target_date = get_input(
                "Date pour orchestrateur (YYYY-MM-DD)", date.today().isoformat()
            )
