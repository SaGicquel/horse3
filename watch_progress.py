...
print(f"\n🔍 ENRICHISSEMENT:")
print(f"   ├─ course_id        : {course_id:,}/{total:,} ({int(course_id*100/total if total > 0 else 0)}%)")
print(f"   ├─ classe_course    : {classe:,}/{total:,} ({int(classe*100/total if total > 0 else 0)}%)")
print(f"   ├─ meteo_code       : {meteo:,}/{total:,} ({int(meteo*100/total if total > 0 else 0)}%)")
print(f"   ├─ handicap_distance: {handicap:,}/{total:,} ({int(handicap*100/total if total > 0 else 0)}%)")
print(f"   └─ entraineur_wr_90j: {winrate:,}/{total:,} ({int(winrate*100/total if total > 0 else 0)}%)")
