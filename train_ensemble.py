...
def create_base_models(X_train, y_train, xgb_params=None, lgb_params=None):
    ...
    return [("rf", rf), ("xgb", xgb_model), ("lgb", lgb_model)]
...
def train_stacking(X_train, y_train, X_val, y_val, base_models):
    ...
    meta_model = LogisticRegression(random_state=42, max_iter=1000, n_jobs=-1)
...
def main():
    ...
    parser.add_argument(
        "--input-dir", type=str, default="data/normalized", help="Répertoire données normalisées"
    )
    parser.add_argument(
        "--optimization-dir",
        type=str,
    ...
    parser.add_argument(
        "--output-dir", type=str, default="data/models", help="Répertoire sauvegarde modèles"
    )
    ...
    try:
        ...
        results = {
            "stacking": {"validation": metrics_stacking_val, "test": metrics_stacking_test},
            "voting": {"validation": metrics_voting_val, "test": metrics_voting_test},
        }
