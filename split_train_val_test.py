...
class DataSplitter:
    ...
    def split_temporal(self, df: pd.DataFrame) -> tuple:
        ...
        logger.info(f"   Train      : {df_train['date_course'].min()} → {df_train['date_course'].max()}")
        logger.info(f"   Validation : {df_val['date_course'].min()} → {df_val['date_course'].max()}")
        logger.info(f"   Test       : {df_test['date_course'].min()} → {df_test['date_course'].max()}")
        logger.info("")
        
        # Distribution target
        logger.info("🎯 DISTRIBUTION VICTOIRES:")
        logger.info(f"   Train      : {df_train['victoire'].sum():5,} / {len(df_train):6,} ({100*df_train['victoire'].mean():.2f}%)")
        logger.info(f"   Validation : {df_val['victoire'].sum():5,} / {len(df_val):6,} ({100*df_val['victoire'].mean():.2f}%)")
        logger.info(f"   Test       : {df_test['victoire'].sum():5,} / {len(df_test):6,} ({100*df_test['victoire'].mean():.2f}%)")
        logger.info("")
