...
class BettingAdvisor:
    ...
    def _load_global_stats(self):
        ...
        logger.info(f"Stats globales chargées: taux victoire moyen = {self.stats_globales['taux_victoire_moyen']:.2f}%")
    
    def get_cheval_stats(self, nom_norm: str, distance: int = None, hippodrome: str = None) -> ChevalStats:
    ...
    def calculer_score_forme(self, stats: ChevalStats) -> Tuple[float, List[str], List[str]]:
        ...
        if stats.nb_victoires_recentes >= 2:
            score += 30
            signaux_pos.append(f"🔥 {stats.nb_victoires_recentes} victoires sur 5 dernières courses!")
    ...
    def calculer_score_jockey(self, stats: JockeyStats, cheval_nom: str) -> Tuple[float, List[str], List[str]]:
        ...
        if stats.courses_avec_cheval >= 3 and stats.victoires_avec_cheval >= 1:
            score += 15
            signaux_pos.append(f"🤝 Bonne association ({stats.victoires_avec_cheval}/{stats.courses_avec_cheval})")
        
        return max(0, min(100, score)), signaux_pos, signaux_neg
    
    def calculer_score_entraineur(self, stats: EntraineurStats) -> Tuple[float, List[str], List[str]]:
    ...
    def calculer_score_conditions(self, stats: ChevalStats, distance: int, hippodrome: str) -> Tuple[float, List[str], List[str]]:
        ...
        if stats.perf_distance:
            if stats.perf_distance['taux_victoire'] > stats.taux_victoire * 1.5:
                score += 15
                signaux_pos.append(f"📏 Excellent sur cette distance ({stats.perf_distance['taux_victoire']:.1f}%)")
        ...
        if stats.perf_hippodrome:
            if stats.perf_hippodrome['taux_victoire'] > stats.taux_victoire * 1.5:
                score += 15
                signaux_pos.append(f"🏟️ Bon sur cet hippodrome ({stats.perf_hippodrome['taux_victoire']:.1f}%)")
            elif stats.perf_hippodrome['nb_courses'] >= 3 and stats.perf_hippodrome['taux_victoire'] == 0:
                score -= 10
    ...
    def calculer_score_tendance(self, tendance: str, amplitude: float, est_favori: bool) -> Tuple[float, List[str], List[str]]:
    ...
    def analyser_participant(self, data: dict, distance: int = None, hippodrome: str = None) -> ParticipantAnalyse:
        """Analyse complète d'un participant"""
        ...
        score, pos, neg = self.calculer_score_tendance(
            data.get('tendance_cote'),
            data.get('amplitude_tendance'),
            data.get('est_favori')
        )
    ...
    def generer_recommandations(self, race_key: str) -> List[RecommandationPari]:
        ...
        for p in participants:
            if p.cote and p.cote < 5 and p.score_global >= 60 and p.confiance in ['HAUTE', 'MOYENNE']:
                recommandations.append(RecommandationPari(
                    type_pari="SUR",
                    niveau_risque=1,
                    participant=p,
                    race_key=race_key,
...
def main():
    ...
    for (race_key,) in courses:
        ...
        for t in rapport['top_3']:
            print(f"  {t['rang']}. {t['nom']} | Score: {t['score']:.1f} | Cote: {t['cote']} | {t['confiance']}")
