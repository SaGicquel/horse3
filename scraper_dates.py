...
def parse_date(date_str):
    ...
    try:
        ...
        raise ValueError(f"Format de date invalide: {date_str}. Utilisez YYYY-MM-DD (ex: 2024-01-15)")
  ...
  %(prog)s --last-month                         # 30 derniers jours
    ...
    if len(results) > 1:
        ...
        for r in results:
            ...
            if r['success']:
                print(f"  {status} {r['date']}: {r['reunions']} réunions, {r['courses']} courses, {r['chevaux']} chevaux")
            else:
