...
def check_october_data():
    ...
    print(f"✅ SCRAPER 1 (Métadonnées):     {meta:,}/{total:,} ({meta*100//total if total > 0 else 0}%)")
    ...
    print(f"✅ SCRAPER 2 (Détails cheval):  {details:,}/{total:,} ({details*100//total if total > 0 else 0}%)")
    ...
    print(f"✅ SCRAPER 3 (Statistiques):    {stats:,}/{total:,} ({stats*100//total if total > 0 else 0}%)")
    ...
    print(f"✅ SCRAPER 4 (Conditions jour):  {conditions:,}/{total:,} ({conditions*100//total if total > 0 else 0}%)")
    ...
    print(f"✅ SCRAPER 5 (Cotes marché):    {cotes:,}/{total:,} ({cotes*100//total if total > 0 else 0}%)")
    ...
    print(f"✅ SCRAPER 7 (Features ML):     {ml:,}/{total:,} ({ml*100//total if total > 0 else 0}%)")
    ...
    print(f"✅ SCRAPER 8 (Connections):     {conn_data:,}/{total:,} ({conn_data*100//total if total > 0 else 0}%)")
