...
def display_progress(stats, previous=None):
    ...
    if stats['perfs_2024'] > 0:
        ...
        print("-" * 90)
        print(f"   Musique : {stats['mus_2024']:6,} / {stats['perfs_2024']:6,} ({mus_2024_pct:6.2f}%)")
        print()
...
if __name__ == '__main__':
    ...
    try:
        ...
        musique_pct = (final_stats['musique'] / final_stats['perfs'] * 100) if final_stats['perfs'] > 0 else 0
        temps_pct = (final_stats['temps'] / final_stats['perfs'] * 100) if final_stats['perfs'] > 0 else 0
        
        print("=" * 90)
        print("📊 RÉSUMÉ FINAL")
        print("=" * 90)
        print(f"Musique : {final_stats['musique']:,} / {final_stats['perfs']:,} ({musique_pct:.2f}%)")
