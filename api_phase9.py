...
class ModelServer:
    ...
    def predict(self, horse_id, jockey_id, entraineur_id):
        ...
        if c_idx is None or j_idx is None or e_idx is None:
            missing = []
            if c_idx is None: missing.append(f"Cheval {horse_id}")
            if j_idx is None: missing.append(f"Jockey {jockey_id}")
            if e_idx is None: missing.append(f"Entraineur {entraineur_id}")
        ...
        with torch.no_grad():
            ...
            logits = self.model(self.graph_data.x_dict, self.graph_data.edge_index_dict, (c_tensor, j_tensor, e_tensor))
