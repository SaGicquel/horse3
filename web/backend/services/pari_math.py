...
def ev_parimutuel_win(p_win: float, market_odds: float, takeout: float = None) -> float:
    """
...
def ev_parimutuel_win_pure(p_win: float, market_odds: float, takeout: float = None) -> float:
    """
...
def ev_parimutuel_place(
    p_place: float, market_odds_place: float, takeout: float = None, nb_partants: int = None
) -> float:
    """
...
def ev_bookmaker(p: float, odds: float, commission: float = 0.05) -> float:
    """
...
def kelly_stake(
    ...
    """
    kelly = kelly_fraction(p, odds, fraction, max_stake_pct, parimutuel, takeout)
...
def is_value_bet(
    p: float, odds: float, cutoff: float = 0.05, parimutuel: bool = True, takeout: float = None
) -> Tuple[bool, float]:
    """
...
def main():
    ...
    for p, odds, desc in test_cases:
        ...
        print(
            f"  p={p:.0%}, odds={odds:.1f}: EV={ev:+.2%} (pure:{ev_pure:+.2%}) "
    ...
    for p, odds, _ in test_cases[:5]:
        ...
        print(
            f"  p={p:.0%}, odds={odds:.1f}: kelly={kelly:.2%}, stake={stake:.0f}€ (bankroll=1000€)"
        )
