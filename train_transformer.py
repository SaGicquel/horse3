...
def load_data(path: str) -> TensorDataset:
    ...
    print(
        f"Données chargées. Shape des séquences: {sequences.shape}, Shape des labels: {labels.shape}"
    )
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if val_auc > best_val_auc:
            ...
            print(
                f"✨ Nouveau meilleur modèle sauvegardé avec AUC = {best_val_auc:.4f} à {MODEL_SAVE_PATH}"
            )
