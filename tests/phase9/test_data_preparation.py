...
def temporal_data():
    ...
    if not os.path.exists(TEMPORAL_DATA_PATH):
        pytest.fail(
            f"Fichier de données temporelles non trouvé: {TEMPORAL_DATA_PATH}. Exécutez d'abord prepare_temporal_data.py."
        )
...
def graph_data():
    ...
    if not os.path.exists(GRAPH_DATA_PATH):
        pytest.fail(
            f"Fichier de données de graphe non trouvé: {GRAPH_DATA_PATH}. Exécutez d'abord build_graph_data.py."
        )
...
class TestTemporalData:
    def test_temporal_output_exists(self):
        ...
        assert os.path.exists(
            TEMPORAL_DATA_PATH
        ), f"Le fichier {TEMPORAL_DATA_PATH} devrait exister."
    ...
    def test_padding_and_mask(self, temporal_data):
        ...
        if padded_sample_idx != -1:
            ...
            assert torch.all(
                padded_values < -0.99
            ), "Les valeurs de padding ne sont pas toutes proches de -1"
