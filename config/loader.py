...
def load_config(path: Optional[str] = None) -> ProBettingConfig:
    ...
    calibration = CalibrationConfig(
        ...
        blend_alpha_obstacle=blend_data.get(
            "alpha_obstacle", cal_data.get("blend_alpha_obstacle", 0.4)
        ),
    ...
    exotics = ExoticsConfig(
        p_place_simulations=exo_data.get(
            "p_place_simulations", sim_data.get("num_simulations", 20000)
        ),
...
def update_config_from_calibration(
    temperature: float, blend_alpha: float, calibrator: str = "platt"
) -> bool:
    """
