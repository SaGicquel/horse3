...
class FeatureNormalizer:
    ...
    def identify_features(self, df: pd.DataFrame):
        ...
        onehot_patterns = ["discipline_", "sexe_", "piste_", "etat_", "hippodrome_"]
    ...
    def fit(self, df_train: pd.DataFrame):
        ...
        logger.info(
            f"   ✅ Scaler fitted sur {X_imputed.shape[1]} features ({len(self.numeric_features)} demandées)"
        )
        logger.info(
            f"   📊 Stats train : mean={self.scaler.mean_[:5]}, std={self.scaler.scale_[:5]}"
        )
        logger.info("")
    ...
    def transform(self, df: pd.DataFrame, split_name: str) -> pd.DataFrame:
        ...
        transformed_cols = [col for col in self.numeric_features if df[col].notna().any()]
        ...
        if len(transformed_cols) == X_scaled.shape[1]:
            ...
            logger.warning(
                f"   ⚠️  Mismatch colonnes: {len(transformed_cols)} vs {X_scaled.shape[1]}"
            )
        ...
        for col in self.numeric_features[:5]:  # Check 5 premières
            ...
            if len(vals) > 0:
                logger.info(
                    f"      {col:<30} : mean={vals.mean():.3f}, std={vals.std():.3f}, min={vals.min():.3f}, max={vals.max():.3f}"
                )
...
def main():
    ...
    parser.add_argument(
        "--input-dir", default="data/splits", help="Répertoire avec train/val/test.csv"
    )
    parser.add_argument("--output-dir", default="data/normalized", help="Répertoire de sortie")
