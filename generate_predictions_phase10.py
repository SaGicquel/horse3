...
def main():
    ...
    logger.info(f"   Jeu de test: {len(test_df)} lignes (du {test_df['date_course'].min()} au {test_df['date_course'].max()})")
    ...
    with torch.no_grad():
        for course_id, group in tqdm(grouped):
            ...
            try:
                ...
                x_dict_batch = {
                    'cheval': X,
                    'jockey': torch.zeros((len(X), 2)).to(device), # Dimension 2 (taux_victoires, taux_places)
                ...
                edge_index_dict_batch = {k: torch.empty((2, 0), dtype=torch.long).to(device) for k in graph_data.edge_index_dict.keys()}
                
                # Appel avec le dictionnaire construit
                out = model(x_dict_batch, edge_index_dict_batch, (h_idx_batch, j_idx_batch, t_idx_batch))
