...
class EnrichisseurZoneTurf:
    ...
    def _enrich_performance(self, id_course: str, data: Dict):
        ...
        numero_corde = self._parse_int(
            data.get("numero", data.get("numero_corde", data.get("corde")))
        )
        ...
        # Cote départ
        if cote_sp := self._parse_float(
            data.get("cote", data.get("cote_sp", data.get("cote_depart")))
        ):
    ...
    def _find_performance(
        self, id_course: str, numero_corde: int, nom_cheval: str
    ) -> Optional[int]:
        """
