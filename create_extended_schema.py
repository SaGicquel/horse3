...
def create_extended_schema(con):
    ...
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_participants_horse ON race_participants(pmu_horse_id);"
    )
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_participants_name ON race_participants(horse_name_norm);"
    )
    ...
    tables = ["race_meetings", "races", "race_participants", "race_betting", "race_incidents"]
