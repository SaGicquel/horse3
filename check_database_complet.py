...
def check_database():
    ...
    print(
        f"   Avec position arrivée       : {row[1]:6d} ({row[1]*100/total if total > 0 else 0:.1f}%)"
    )
    print(
        f"   Avec cote                   : {row[2]:6d} ({row[2]*100/total if total > 0 else 0:.1f}%)"
    )
    print(
        f"   Avec musique                : {row[3]:6d} ({row[3]*100/total if total > 0 else 0:.1f}%)"
    )
    print(
        f"   Avec temps                  : {row[4]:6d} ({row[4]*100/total if total > 0 else 0:.1f}%)"
    )
    ...
    print(
        f"   Avec nom père               : {row[1]:6d} ({row[1]*100/total if total > 0 else 0:.1f}%)"
    )
    print(
        f"   Avec nom mère               : {row[2]:6d} ({row[2]*100/total if total > 0 else 0:.1f}%)"
    )
    print(
        f"   Avec propriétaire           : {row[3]:6d} ({row[3]*100/total if total > 0 else 0:.1f}%)"
    )
    ...
    if chevaux_sans_stats > 0:
        recommendations.append(
            f"Recalculer stats pour {chevaux_sans_stats} chevaux : python calcul_stats.py --all"
        )
    ...
    if last_date:
        ...
        if days_ago > 1:
            ...
            recommendations.append("Commande : python scraper_pmu_adapter.py --date today")
