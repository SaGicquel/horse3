...
def simple_isotonic_calibration(probabilities, true_labels, test_probabilities):
    ...
    logger.info(f"✅ Calibration isotonique appliquée")
...
def apply_simple_calibration(df, params):
    ...
    # Blend modèle/marché
    result_df['p_blend'] = (
        alpha * result_df['p_calibrated'] + 
        (1 - alpha) * result_df['p_market_corrected']
    )
...
def main():
    ...
    try:
        ...
        logger.info(f"🔍 Courses normalisées: {perfect_sums:,}/{len(race_sums):,} ({perfect_sums/len(race_sums)*100:.1f}%)")
        
        # Distribution des probabilités
        for col in ['p_model_win', 'p_final']:
            stats = output_df[col].describe()
            logger.info(f"📈 {col}: min={stats['min']:.4f}, médiane={stats['50%']:.4f}, max={stats['max']:.4f}")
