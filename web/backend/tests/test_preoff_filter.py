...
class TestStatusFiltering:
    ...
    EXCLUDED_STATUSES = [
        "NON_PARTANT", "NON PARTANT", "NONPARTANT", "NP",
        "DAI", "DIA", "DISQUALIFIE", "DISQ", "DQ",
        "ARR", "ARRET", "ARRETE", "STOPPED",
        "CHU", "CHUTE", "TOMBE", "TOM", "FALL",
        "RET", "RETIRE", "RETIREE", "RÉTIRÉ", "WITHDRAWN",
        "DIST", "DISTANCIE", "DISTANCED"
    ]
    
    # Statuts qui doivent être ACCEPTÉS
    ALLOWED_STATUSES = [
        "PARTANT", "PARTANTE", "PART", "P", "", None
    ]
    ...
    def is_excluded(statut_norm):
        ...
        return statut_norm not in allowed_status or any(key in statut_norm for key in EXCLUDED_KEYWORDS)
    ...
    def test_excluded_statuses_are_rejected(self):
        ...
        for statut in self.EXCLUDED_STATUSES:
            ...
            assert self.is_excluded(statut_norm), f"Le statut '{statut}' devrait être exclu mais ne l'est pas"
    ...
    def test_allowed_statuses_are_accepted(self):
        ...
        for statut in self.ALLOWED_STATUSES:
            ...
            if statut_norm in {"", "PARTANT", "PARTANTE", "PART", "P"}:
                assert not self.is_excluded(statut_norm), f"Le statut '{statut}' devrait être accepté mais est exclu"
...
class TestOddsFiltering:
    ...
    def test_cote_reference_also_filtered(self):
        ...
        assert effective_odds > self.MAX_PREOFF_COTE, "Ce cas doit être filtré par le code principal"
