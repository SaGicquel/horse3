...
def verify_enrichment():
    ...
    print(f"📋 SCRAPER 1 - Métadonnées course")
    ...
    print(f"   Autostart: {autostart}/{total_parts} ({autostart*100//total_parts if total_parts else 0}%)")
    print()
    ...
    print(f"🐴 SCRAPER 2 - Détails cheval")
    print(f"   Numéro PMU: {num}/{total_parts} ({num*100//total_parts if total_parts else 0}%)")
    print(f"   Poids porté: {poids}/{total_parts} ({poids*100//total_parts if total_parts else 0}%)")
    print(f"   Ferrure: {ferrure}/{total_parts} ({ferrure*100//total_parts if total_parts else 0}%)")
    print(f"   Matériel: {materiel}/{total_parts} ({materiel*100//total_parts if total_parts else 0}%)")
    print(f"   Days off: {days_off}/{total_parts} ({days_off*100//total_parts if total_parts else 0}%)")
    print()
    ...
    print(f"🏁 SCRAPER 4 - Conditions jour")
    print(f"   Pace début: {pace_d}/{total_parts} ({pace_d*100//total_parts if total_parts else 0}%)")
    print(f"   Pace milieu: {pace_m}/{total_parts} ({pace_m*100//total_parts if total_parts else 0}%)")
    print(f"   Pace fin: {pace_f}/{total_parts} ({pace_f*100//total_parts if total_parts else 0}%)")
    print(f"   Pénétromètre: {penetro}/{total_parts} ({penetro*100//total_parts if total_parts else 0}%)")
    print()
    ...
    print(f"💰 SCRAPER 5 - Cotes marché")
    print(f"   Évolution cote: {evol}/{total_parts} ({evol*100//total_parts if total_parts else 0}%)")
    print(f"   Tendance: {tendance}/{total_parts} ({tendance*100//total_parts if total_parts else 0}%)")
    ...
    print(f"🤖 SCRAPER 7 - Features ML")
    print(f"   Indice confiance: {confiance}/{total_parts} ({confiance*100//total_parts if total_parts else 0}%)")
    print(f"   Probabilité victoire: {proba_win}/{total_parts} ({proba_win*100//total_parts if total_parts else 0}%)")
    print(f"   Kelly criterion: {kelly}/{total_parts} ({kelly*100//total_parts if total_parts else 0}%)")
    ...
    row = cur.fetchone()
    if row:
        print(f"📝 EXEMPLE DE LIGNE ENRICHIE")
