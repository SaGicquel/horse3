...
class SafeMLFeatureEngineer:
    ...
    def add_safe_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df = df.drop(
            [
                "nb_courses_historiques",
                "nb_victoires_historiques",
                "taux_victoires_historique",
                "taux_places_historique",
            ],
            axis=1,
        )
...
def main():
    ...
    try:
        ...
        safe_cols = [
            col
            for col in df.columns
            if col not in ["position_arrivee", "victoire", "place"] + dangerous_cols
        ]

        logger.info(f"   - Colonnes SAFE: {len(safe_cols)}")
        logger.info("   - Targets: 3 (position_arrivee, victoire, place)")
