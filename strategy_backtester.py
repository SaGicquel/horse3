...
def load_real_predictions(
    ...
    if config is not None:
        ...
        df = df.loc[mask].copy()
        print(f"   Après filtre temporel {config.test_start} → {config.test_end}: {len(df):,} entrées")
    ...
    def _adapt_real_data_format(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if hasattr(self.config, 'disciplines') and self.config.disciplines:
            if 'discipline' in df_adapted.columns:
                ...
                print(f"   Après filtre disciplines {self.config.disciplines}: {len(df_adapted):,} entrées")
        
        # Filtrer les cotes
        if hasattr(self.config, 'min_odds') and hasattr(self.config, 'max_odds'):
            mask = (
                (df_adapted['odds_preoff'] >= self.config.min_odds) &
                (df_adapted['odds_preoff'] <= self.config.max_odds)
            )
            df_adapted = df_adapted[mask].copy()
            print(f"   Après filtre cotes [{self.config.min_odds}-{self.config.max_odds}]: {len(df_adapted):,} entrées")
