...
def learn_temperature_scipy(
    ...
    return {
        ...
        'improvement_pct': round((optimal_ll - baseline_ll) / abs(baseline_ll) * 100, 2) if baseline_ll != 0 else 0,
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Nombre minimum de courses"
    )
    parser.add_argument(
        '--output', '-o',
        default='results/pl_temperatures.json',
        help="Fichier de sortie"
    )
    parser.add_argument(
        '--cross-validate',
        action='store_true',
        help="Faire une validation croisée"
    )
    ...
    races = extract_historical_races(
        discipline=None if args.discipline == 'all' else args.discipline,
        days_back=args.days
    )
    ...
    if args.cross_validate:
        ...
        for disc in ['plat', 'trot', 'obstacle']:
            ...
            if 'error' not in cv_result:
                ...
                print(f"  Moyenne: {cv_result['mean_temperature']} ± {cv_result['std_temperature']}")
