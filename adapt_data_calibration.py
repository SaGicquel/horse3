...
def adapt_data_for_calibration():
    ...
    logger.info(f"   📈 Logits - Médiane: {logits_stats['50%']:.3f}, Moyenne: {logits_stats['mean']:.3f}")
    ...
    logger.info(f"   🎯 Probabilités - Médiane: {prob_stats['50%']:.4f}, Moyenne: {prob_stats['mean']:.4f}")
    ...
    logger.info(f"   🏇 Distribution disciplines:")
    ...
    logger.info(f"   🐎 Chevaux par course: {horses_per_race.mean():.1f} (min: {horses_per_race.min()}, max: {horses_per_race.max()})")
    ...
    output_cols = [
        'race_id', 'id_cheval', 'date', 'date_course',
        'logits_model', 'p_model_win',  # Modèle
        'label_win', 'place', 'position_arrivee',  # Targets
        'odds_market_preoff', 'cote_sp',  # Marché
        'split', 'discipline'  # Méta
    ]
    ...
    logger.info(f"💾 Sauvegarde des données adaptées...")
    df_output.to_csv(output_path, index=False)
    
    logger.info(f"✅ Données adaptées sauvegardées: {output_path}")
    logger.info(f"📊 Dimensions finales: {len(df_output):,} lignes × {len(df_output.columns)} colonnes")
