...
def generate_exotic_analysis(
    ...
    estimator = PlaceProbabilityEstimator(
        p_win, discipline=discipline, config=config, horse_names=horse_names
    )
    ...
    result = {
        ...
        "input": {
            "p_win_blend": {horse_names[i]: round(float(p_win[i]), 4) for i in range(n_horses)}
        },
    }
    ...
    result["p_place"] = {
        horse_names[i]: round(float(place_data["p_place"][i]), 4) for i in range(n_horses)
    }
    ...
    ev_data = estimator.calculate_tickets_ev(combo_data["combo_probs"], structure, fixed_odds)
    result["ev_analysis"] = {
    ...
    result["packs"] = generate_ticket_packs(ev_data["positive_ev_tickets"], config)

    return result


def generate_ticket_packs(
    positive_ev_tickets: List[Dict], config: PlaceEstimatorConfig
) -> Dict[str, Any]:
    """
    ...
    def make_pack(tickets: List[Dict], label: str, max_tickets: int = 10) -> Dict:
        ...
        return {
            ...
            "risk_profile": "LOW"
            if label == "SÛR"
            else ("MEDIUM" if label == "ÉQUILIBRÉ" else "HIGH"),
        }
...
def integrate_with_exotic_generator(
    ...
    estimator = PlaceProbabilityEstimator(
        p_win_blend, discipline=discipline, config=config, horse_names=horse_names
    )
    ...
    result = generator.generate(
        probabilities=list(p_win_blend), horse_names=horse_names, structure=structure, budget=budget
    )
...
def main():
    ...
    parser.add_argument("--race-key", "-r", help="Clé de course (format: YYYY-MM-DD|Rx|Cx|HIP)")
    parser.add_argument(
        "--discipline",
        "-d",
        default="plat",
    ...
    parser.add_argument(
        "--simulations", "-n", type=int, default=20000, help="Nombre de simulations"
    )
    parser.add_argument("--output", "-o", help="Fichier de sortie JSON")
    parser.add_argument(
        "--demo", action="store_true", help="Mode démonstration avec données exemple"
    )
    ...
    if args.demo:
        ...
        print("-" * 40)
        print(
            f"  Tickets EV+: {result['ev_analysis']['n_positive_ev']} / {result['ev_analysis']['n_tickets']}"
        )
        ...
        for pack_name, pack in result["packs"].items():
            if isinstance(pack, dict) and "n_tickets" in pack:
                print(
                    f"  {pack_name}: {pack['n_tickets']} tickets, "
        ...
        print(
            "Utilisez --demo pour une démonstration, ou --race-key pour analyser une course réelle"
        )
