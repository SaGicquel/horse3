...
def check_calibration_health():
    ...
    try:
        ...
        if brier > 0.25 or ece > 0.10:
            logger.warning(
                f"⚠️ Métriques dégradées (Brier={brier:.3f}, ECE={ece:.3f}) - recalibration requise"
            )
            return False
...
def main():
    ...
    if not run_command("python update_results.py", "Mise à jour P&L", critical=False):
        logger.warning(
            "⚠️ Problème lors de la mise à jour des résultats (peut-être normal si pas de paris en cours)"
        )
    ...
    if weekday == 6 or needs_recalibration:
        ...
        if not run_command(
            "python cli.py calibrate --days 30", "Recalibration modèle", critical=False
        ):
    ...
    # ========================================
    if not run_command(
        f"python prepare_daily_features.py --date {today}", "Calcul features du jour"
    ):
    ...
    # ========================================
    if not run_command(
        f"python cli.py pick --date {today}", "Génération pronostics", critical=False
    ):
    ...
    # ========================================
    if not run_command(
        "python paper_trading_v2.py --input data/daily_features.csv",
        "Génération paris paper trading",
    ):
    ...
    try:
        ...
        if result:
            ...
            run_command(
                f"python cli.py exotic --race {race_key}",
                "Génération tickets exotiques",
                critical=False,
            )
        else:
