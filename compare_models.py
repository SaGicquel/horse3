...
class ModelComparator:
    ...
    def load_feedback_data(self, days: int = 7) -> pd.DataFrame:
        ...
        champion_data = {
            ...
            'actual_position': np.random.choice([1, 2, 3, 4, 5], n_champion, p=[0.12, 0.15, 0.18, 0.25, 0.30])
        }
        ...
        challenger_data = {
            ...
            'actual_position': np.random.choice([1, 2, 3, 4, 5], n_challenger, p=[0.18, 0.20, 0.17, 0.22, 0.23])
        }
    ...
    def compare_models(self, df: pd.DataFrame) -> Dict:
        ...
        if len(df_champion) < self.min_samples:
            logger.error(f"❌ Pas assez de prédictions champion: {len(df_champion)} < {self.min_samples}")
            return {
        ...
        if len(df_challenger) < self.min_samples:
            logger.error(f"❌ Pas assez de prédictions challenger: {len(df_challenger)} < {self.min_samples}")
            return {
        ...
        logger.info(f"   Champion: ROC-AUC={champion_roc_auc:.4f}, Brier={champion_brier:.4f}, n={len(df_champion)}")
        
        # Calculer métriques challenger
        logger.info("   📈 Calcul métriques challenger...")
        challenger_roc_auc = self.calculate_roc_auc(
            df_challenger['victoire'].values,
        ...
        logger.info(f"   Challenger: ROC-AUC={challenger_roc_auc:.4f}, Brier={challenger_brier:.4f}, n={len(df_challenger)}")
        ...
        comparison = {
            ...
            'brier_improvement': round(champion_brier - challenger_brier, 4)  # Brier: plus petit = mieux
        }
        ...
        if should_promote:
            ...
            logger.info(f"   ✅ RECOMMANDATION: PROMOUVOIR CHALLENGER")
        else:
            ...
            logger.info(f"   ⏸️  RECOMMANDATION: CONSERVER CHAMPION")
    ...
    def promote_challenger(self, dry_run: bool = False) -> bool:
        ...
        if dry_run:
            ...
            logger.info(f"   3. Mise à jour metadata")
            return True
        ...
        try:
            ...
            logger.info(f"   📋 Copie challenger → champion")
            if champion_dir.exists():
    ...
    def generate_report(self, comparison_result: Dict, output_path: str = "comparison_report.json") -> None:
...
Exemples:
    ...
    parser.add_argument(
        ...
        type=int,
        default=7,
        help='Nombre de jours de feedback à analyser (défaut: 7)'
    )
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Simuler la promotion sans faire de changements'
    )
    ...
    logger.info(f"   Significatif (stat)    : {'✅ OUI' if result['comparison']['significant'] else '❌ NON'}")
    logger.info(f"   Significatif (pratique): {'✅ OUI' if result['comparison']['practical_significant'] else '❌ NON'}")
    logger.info("")
