...
def extract_features_and_metadata(df):
    ...
    meta_cols = ['race_key', 'id_cheval', 'date_course', 'is_win', 'place', 
                 'cote_sp', 'cote_pm', 'nom_cheval', 'hippodrome', 'numero_corde']
    
    # Colonnes à exclure des features (métadonnées + colonnes texte)
    exclude_cols = ['race_key', 'id_cheval', 'date_course', 'is_win', 'place', 
                    'nom_cheval', 'hippodrome', 'jockey', 'entraineur', 'proprietaire',
                    'musique', 'commentaire', 'temps_secteur', 'temps_final',
                    'id_course', 'sexe_cheval', 'discipline', 'etat_piste', 
                    'nom_hippodrome', 'type_piste', 'hippodrome_ville', 'hippodrome_top20',
                    'ecart']
...
def generate_predictions(model, datasets):
    ...
    for split_name, data_dict in datasets.items():
        ...
        logger.info(f"🔮 Prédictions sur {split_name} ({X.shape[0]} lignes, {X.shape[1]} features)...")
        
        # Prédire les probabilités
        try:
            # XGBoost retourne les probas de classe 1 (victoire)
            pred_probs = model.predict_proba(X)[:, 1]
            logger.info(f"   ✅ Prédictions générées (min: {pred_probs.min():.3f}, max: {pred_probs.max():.3f})")
...
def save_predictions(predictions_df, output_path):
    ...
    if 'date_course' in predictions_df.columns:
        predictions_df = predictions_df.sort_values(['date_course', 'p_model_win'], ascending=[True, False])
    ...
    if 'p_model_win' in predictions_df.columns:
        logger.info(f"📊 Statistiques prédictions :")
