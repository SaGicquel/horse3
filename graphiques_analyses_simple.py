...
def graph_distribution_sexe():
    ...
    wedges, texts, autotexts = ax1.pie(df['nb_chevaux'], labels=df['sexe'], autopct='%1.1f%%',
...
def graph_distribution_pays():
    ...
    bars = ax.barh(df['pays_naissance'], df['nb_chevaux'], color=colors, edgecolor='black', linewidth=1)
...
def graph_top_entraineurs():
    ...
    bars = ax.barh(df['entraineur_courant'], df['nb_chevaux'], color=colors, edgecolor='black', linewidth=1)
    ...
    print(f"\n📈 Statistiques:")
    print(f"   • Entraîneur #1: {df.iloc[0]['entraineur_courant']} ({df.iloc[0]['nb_chevaux']:,} chevaux)")
...
def graph_top_chevaux_victoires():
    ...
    fig, ax = plt.subplots(figsize=(14, 10))
    colors = plt.cm.viridis(np.linspace(0.3, 0.9, len(df)))
    bars = ax.barh(df['nom_court'], df['nombre_victoires_total'], color=colors, edgecolor='black', linewidth=1)
...
def graph_dashboard():
    ...
    for titre, valeur, couleur, (row, col) in kpis:
        ...
        ax.text(0.5, 0.6, f'{valeur:,.0f}', ha='center', va='center', 
                fontsize=32, fontweight='bold', color=couleur)
    ...
    taux_victoire = (kpi['total_victoires'].iloc[0] / kpi['total_courses'].iloc[0] * 100) if kpi['total_courses'].iloc[0] > 0 else 0
    info_text = f"""
