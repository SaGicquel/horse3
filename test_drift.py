...
def test_drift_detection():
    ...
    tests_total += 1
    if run_command(
        "python detect_drift.py --help",
        "Affichage aide du script"
    ):
        tests_passed += 1
    ...
    if run_command(
        ...
        if Path("test_drift_report.json").exists():
            ...
            with open("test_drift_report.json") as f:
                ...
                required_keys = ['timestamp', 'total_features', 'features_with_drift', 
                                'critical_drifts', 'warning_drifts', 'drift_percentage', 'features']
